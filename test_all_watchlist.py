    ...
    for symbol in watchlist:
        ...
        try:
            ...
            if df.empty or len(df) < 26:
                print(f"❌ {symbol}: Insufficient data ({len(df) if not df.empty else 0} bars)")
                continue
            ...
            if current_price < config['MIN_PRICE'] or current_price > config['MAX_PRICE']:
                print(f"❌ {symbol}: Price ${current_price:.2f} outside limits (${config['MIN_PRICE']}-${config['MAX_PRICE']})")
                continue
            ...
            for strategy_name, strategy in strategies.items():
                try:
                    ...
                    if signal:
                        signals_found = True
                        print(f"🚨 {strategy_name} SIGNAL: {signal['action']} - {signal['reason']}")
                        ...
                        print(f"   📈 Confidence: {confidence:.1f}% ({'✅ PASS' if should_trade else '❌ FAIL'})")
            ...
            if not signals_found:
                ...
                if bb_upper > 0:
                    ...
                    print(f"   BB Position: {bb_position:.1f}% (Upper: ${bb_upper:.2f}, Lower: ${bb_lower:.2f})")
                    print(f"   RSI: {rsi:.1f}")
