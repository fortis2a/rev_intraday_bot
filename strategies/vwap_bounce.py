...
class VWAPBounceStrategy:
    ...
    def calculate_volume_profile(self, df: pd.DataFrame, periods: int = 50) -> Dict:
        ...
        for _, row in recent_data.iterrows():
            ...
            if bar_range > 0:
                ...
                bin_index = int((mid_price - recent_data['low'].min()) / bin_size) if bin_size > 0 else 0
    ...
    def detect_vwap_bounce_setup(self, current_price: float, vwap_bands: Dict) -> Dict[str, bool]:
        """Detect VWAP bounce setup conditions using specialized analysis"""
        
        # Distance from VWAP bands
        near_vwap_upper_1 = abs(current_price - vwap_bands['vwap_upper_1']) / current_price < 0.001
        near_vwap_lower_1 = abs(current_price - vwap_bands['vwap_lower_1']) / current_price < 0.001
        near_vwap_upper_2 = abs(current_price - vwap_bands['vwap_upper_2']) / current_price < 0.001
        near_vwap_lower_2 = abs(current_price - vwap_bands['vwap_lower_2']) / current_price < 0.001
    ...
    def generate_signal(self, symbol: str, df: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            indicator_data = unified_indicator_service.get_indicators_for_strategy(df, symbol, 'vwap_bounce')
            ...
            if current_price < volume_profile['poc_price'] and current_price > volume_profile['value_area_low']:
                bullish_signals += 1
                reasons.append("Price near volume support area")
            elif current_price > volume_profile['poc_price'] and current_price < volume_profile['value_area_high']:
                bearish_signals += 1
                reasons.append("Price near volume resistance area")
            
            # Point of Control proximity (our specialty)
            poc_distance = abs((current_price - volume_profile['poc_price']) / volume_profile['poc_price']) * 100
            ...
            if signal == 'BUY':
                ...
                stop_loss = max(vwap_bands['vwap_lower_2'], entry_price * (1 - self.stop_loss_pct))
                take_profit = min(vwap_bands['vwap_upper_1'], entry_price * (1 + self.take_profit_pct))
            elif signal == 'SELL':
                entry_price = current_price
                stop_loss = min(vwap_bands['vwap_upper_2'], entry_price * (1 + self.stop_loss_pct))
                take_profit = max(vwap_bands['vwap_lower_1'], entry_price * (1 - self.take_profit_pct))
            else:
                return None
            ...
            return {
                ...
                'indicators': {
                    ...
                    'volume_ratio': volume_ratio if 'volume_ratio' in locals() else None
                }
            }
    ...
    def analyze_market_data(self, df: pd.DataFrame) -> Dict:
        ...
        if current_price > volume_profile['poc_price'] and current_price < volume_profile['value_area_high']:
            bearish_signals += 1
            reasons.append("Price near volume resistance area")
        ...
        if signal in ['BUY', 'SELL']:
            ...
            if signal == 'BUY':
                entry_price = current_price
                stop_loss = max(current_vwap - (vwap_std * 1.5), entry_price * (1 - self.stop_loss_pct))
                take_profit = min(current_vwap + (vwap_std * 2.0), entry_price * (1 + self.take_profit_pct))
            else:  # SELL
                entry_price = current_price
                stop_loss = min(current_vwap + (vwap_std * 1.5), entry_price * (1 + self.stop_loss_pct))
                take_profit = max(current_vwap - (vwap_std * 2.0), entry_price * (1 - self.take_profit_pct))
        else:
...
def create_vwap_bounce_strategy(symbol: str, timeframe: str = "1min") -> VWAPBounceStrategy:
