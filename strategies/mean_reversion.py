...
class MeanReversionStrategy:
    ...
    def calculate_support_resistance(self, df: pd.DataFrame, window: int = 20) -> Dict[str, float]:
        ...
        resistance = resistance_levels.tail(3).mean() if len(resistance_levels) > 0 else recent_data['high'].max()
        
        # Support: average of recent lows
        support_levels = recent_data[recent_data['low'] == lows]['low'].dropna()
        support = support_levels.tail(3).mean() if len(support_levels) > 0 else recent_data['low'].min()
    ...
    def generate_signal(self, symbol: str, df: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            indicator_data = unified_indicator_service.get_indicators_for_strategy(df, symbol, 'mean_reversion')
            ...
            distance_to_support = ((current_price - sr_levels['support']) / sr_levels['support']) * 100
            distance_to_resistance = ((sr_levels['resistance'] - current_price) / current_price) * 100
            ...
            if signal == 'BUY':
                ...
                stop_loss = max(sr_levels['support'] * 0.995, entry_price * (1 - self.stop_loss_pct))
                take_profit = min(sr_levels['resistance'] * 0.995, entry_price * (1 + self.take_profit_pct))
            elif signal == 'SELL':
                entry_price = current_price
                stop_loss = min(sr_levels['resistance'] * 1.005, entry_price * (1 + self.stop_loss_pct))
                take_profit = max(sr_levels['support'] * 1.005, entry_price * (1 - self.take_profit_pct))
            else:
                return None
            ...
            self.logger.error(f"Mean reversion signal generation failed for {symbol}: {e}")
            return None
    ...
    def analyze_market_data(self, df: pd.DataFrame) -> Dict:
        ...
        # MACD
        macd = self.calculate_macd(close_prices, self.macd_fast, self.macd_slow, self.macd_signal)
        
        # Stochastic
        stoch = self.calculate_stochastic(df['high'], df['low'], df['close'], self.stoch_k, self.stoch_d)
        ...
        # Volume confirmation
        volume_confirmation = volume_indicators['volume_ratio'].iloc[-1] > 1.2  # Above average volume
        ...
        # Generate signals
        if oversold_signals >= 2 and distance_to_support < 2:  # Near support with multiple oversold signals
            signal = 'BUY'
            confidence = min(95, 40 + (oversold_signals * 15) + (20 if volume_confirmation else 0))
            reasons.append("Multiple oversold indicators near support")
            
        elif overbought_signals >= 2 and distance_to_resistance < 2:  # Near resistance with multiple overbought signals
            signal = 'SELL'
            confidence = min(95, 40 + (overbought_signals * 15) + (20 if volume_confirmation else 0))
        ...
        if len(macd['histogram']) > 5:
            ...
            if signal == 'BUY' and recent_histogram.iloc[-1] > recent_histogram.iloc[-2]:
                confidence += 10
                reasons.append("MACD histogram turning positive")
            elif signal == 'SELL' and recent_histogram.iloc[-1] < recent_histogram.iloc[-2]:
                confidence += 10
    ...
    def get_position_size(self, account_value: float, risk_per_trade: float = 0.02) -> float:
...
def create_mean_reversion_strategy(symbol: str, timeframe: str = "1min") -> MeanReversionStrategy:
...
if __name__ == "__main__":
    ...
    sample_data = pd.DataFrame({
        'timestamp': pd.date_range(start='2025-08-16 09:30', periods=100, freq='1min'),
