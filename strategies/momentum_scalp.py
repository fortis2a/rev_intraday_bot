...
class MomentumScalpStrategy:
    ...
    def detect_momentum_patterns(self, indicators: Dict, current_price: float) -> Dict[str, bool]:
    ...
    def generate_signal(self, symbol: str, df: pd.DataFrame) -> Optional[Dict]:
        ...
        try:
            ...
            indicator_data = unified_indicator_service.get_indicators_for_strategy(df, symbol, 'momentum_scalp')
            ...
            if len(indicators['williams_r']) > 0:
                ...
                if current_williams > -20 and patterns['momentum_accelerating']:  # Momentum continuation
                    bullish_signals += 1
            ...
            if len(indicators['roc']) > 0:
                ...
                if current_roc > 1.0:  # Strong positive momentum
                    bullish_signals += 2
                    reasons.append(f"Strong positive momentum (ROC: {current_roc:.2f}%)")
                elif current_roc < -1.0:  # Strong negative momentum
                    bearish_signals += 2
                    reasons.append(f"Strong negative momentum (ROC: {current_roc:.2f}%)")
            ...
            if signal in ['BUY', 'SELL']:
                atr = adx_indicators['atr'].iloc[-1] if len(adx_indicators['atr']) > 0 else current_price * 0.01
            ...
            return {
                ...
                'indicators': {
                    ...
                    'williams_r': current_williams if 'current_williams' in locals() else None,
            ...
            self.logger.error(f"Momentum scalp signal generation failed for {symbol}: {e}")
            return None
    ...
    def should_exit_scalp(self, entry_time: datetime, current_pnl_pct: float) -> Tuple[bool, str]:
...
def create_momentum_scalp_strategy(symbol: str, timeframe: str = "1min") -> MomentumScalpStrategy:
