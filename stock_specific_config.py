...
def meets_confidence_threshold(symbol: str, min_threshold: float = 70.0) -> bool:
    ...
    if not meets_threshold:
        print(
            f"[CONFIDENCE FILTER] {symbol}: {final_confidence:.1f}% < {min_threshold}% - SKIPPING TRADE"
        )
    else:
        print(
            f"[CONFIDENCE FILTER] {symbol}: {final_confidence:.1f}% >= {min_threshold}% - TRADE ALLOWED"
        )
...
def get_filtered_watchlist(
    watchlist: list, min_confidence: float = 75.0, use_real_time: bool = True
) -> list:
    """
    ...
    print(
        f"📊 Mode: {'Real-time calculation (STRICT - no fallback)' if use_real_time else 'Historical baseline'}"
    )
    print("=" * 60)
    ...
    if use_real_time:
        ...
        try:
            ...
            for symbol in watchlist:
                ...
                try:
                    ...
                    result = calculator.calculate_real_time_confidence(
                        symbol, expected_vol
                    )
                    ...
                    if result["confidence"] >= min_confidence:
                        ...
                        print(
                            f"[REAL-TIME] {symbol}: {result['confidence']:.1f}% >= {min_confidence}% - TRADE ALLOWED"
                        )
                    else:
                        print(
                            f"[REAL-TIME] {symbol}: {result['confidence']:.1f}% < {min_confidence}% - SKIPPING TRADE"
                        )

                except Exception as e:
                    print(
                        f"[ERROR] {symbol}: Real-time calculation failed ({e}) - EXCLUDED FROM TRADING"
                    )
            ...
            print(
                "[STRICT POLICY] No fallback allowed - NO TRADING when real-time fails"
            )
        ...
        print(
            "[WARNING] Using historical confidence - NOT recommended for live trading"
        )
        for symbol in watchlist:
    ...
    print(
        f"[OK] Trading: {', '.join(filtered_list) if filtered_list else 'NONE - Real-time required'}"
    )
...
def get_real_time_confidence_for_trade(symbol: str) -> dict:
    ...
    try:
        ...
        print(
            f"[ERROR] Real-time confidence calculator not available for {symbol}: {e}"
        )
        return {
            "symbol": symbol,
...
def should_execute_trade(symbol: str, signal_type: str = "entry") -> dict:
    ...
    direction = (
        "LONG"
        if signal_type == "BUY"
        else "SHORT" if signal_type == "SELL" else signal_type.upper()
    )
    ...
    min_confidence = (
        config.get("MIN_CONFIDENCE_THRESHOLD", 0.75) * 100
    ...
    if execute_trade:
        print(
            f"[OK] EXECUTE TRADE: {symbol} - Real-time confidence: {confidence_data['confidence']:.1f}%"
        )
    else:
        if confidence_data["confidence"] > 0:
            print(
                f"[SKIP] SKIP TRADE: {symbol} - Real-time confidence: {confidence_data['confidence']:.1f}% below {min_confidence:.0f}% threshold"
            )
        else:
            print(
                f"[SKIP] SKIP TRADE: {symbol} - Real-time calculation failed or returned 0% confidence"
            )
...
def print_stock_analysis_summary():
    ...
    print(
        f"{'Stock':<6} {'Stop':<6} {'Profit':<7} {'Trail':<6} {'Volatility':<10} {'Profile':<15}"
    )
    print("-" * 80)

    for symbol, config in STOCK_SPECIFIC_THRESHOLDS.items():
        print(
            f"{symbol:<6} {config['stop_loss_pct']*100:<6.1f} "
