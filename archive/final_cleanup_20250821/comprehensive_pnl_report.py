...
class ComprehensivePnLReportGenerator:
            ...
            for order in orders:
                if hasattr(order, 'filled_at') and order.filled_at:
                    ...
                    if order_date != exclude_date:
                        ...
                        excluded_count += 1
                        excluded_trades_info.append(f"{order.symbol} {order.side} {order.filled_qty}@${order.filled_avg_price}")
            
            print(f"🔍 Total orders retrieved: {len(orders)}")
            print(f"❌ Excluded orders from August 12th: {excluded_count}")
            if excluded_trades_info:
                print(f"📋 Excluded trades: {', '.join(excluded_trades_info[:5])}{'...' if len(excluded_trades_info) > 5 else ''}")
        ...
        try:
            ...
            for order in orders:
                ...
                order_data = {
                    ...
                    'notional': float(order.notional) if hasattr(order, 'notional') and order.notional else filled_qty * filled_price
                }
            ...
            for symbol, trades in symbol_groups.items():
                ...
                for sell in sells:
                    ...
                    while sell_qty_remaining > 0 and buy_queue:
                        ...
                        buy_hour = buy_time.hour if buy_time.tzinfo else (buy_time - timedelta(hours=4)).hour
                        sell_hour = sell_time.hour if sell_time.tzinfo else (sell_time - timedelta(hours=4)).hour
                        
                        # Determine trading period
                        avg_hour = (buy_hour + sell_hour) / 2
                        if 6 <= avg_hour < 11:  # 6 AM - 11 AM ET (10 AM - 3 PM market time)
                            period = 'Morning'
                        ...
                        trade_record = {
                            ...
                            'sell_time': sell_time,
                            'period': period,
                            'hold_duration': (sell_time - buy_time).total_seconds() / 60,  # minutes
    ...
    def calculate_performance_metrics(self, trades):
        ...
        try:
            ...
            avg_loss = abs(np.mean([t['pnl'] for t in trades if t['pnl'] < 0])) if losing_trades > 0 else 0
    ...
    def create_visualizations(self, trades, metrics, output_dir='reports/daily'):
        ...
        try:
            ...
            fig.suptitle('Comprehensive Trading Performance Analysis', 
                        fontsize=28, fontweight='bold', y=0.98)  # Increased from 20
            ...
            fig.text(0.5, 0.96, summary_text, ha='center', fontsize=18, style='italic')  # Increased from 12
            ...
            bars = stock_pnl.plot(kind='bar', color=colors, edgecolor='black', linewidth=1)
            plt.title('P&L by Stock', fontsize=18, fontweight='bold', pad=20)  # Increased from 14
            ...
            for i, v in enumerate(stock_pnl.values):
                color = 'white' if abs(v) > max(abs(stock_pnl.values)) * 0.5 else 'black'
                plt.text(i, v/2, f'${v:.2f}', ha='center', va='center', 
                        fontweight='bold', color=color, fontsize=10)
                plt.text(i, v + (max(stock_pnl.values) * 0.05 if v >= 0 else min(stock_pnl.values) * 0.05), 
            ...
            bars = trade_counts.plot(kind='bar', color='#4682B4', edgecolor='black', linewidth=1)
            plt.title('📊 Trade Volume by Stock', fontsize=14, fontweight='bold', pad=20)
            ...
            colors_wr = ['#228B22' if x >= 50 else '#FF6347' if x >= 30 else '#DC143C' for x in stock_winrate.values]
            stock_winrate.plot(kind='bar', color=colors_wr, edgecolor='black', linewidth=1)
            ...
            # Add benchmark lines
            plt.axhline(y=50, color='gold', linestyle='--', alpha=0.8, linewidth=2, label='Target: 50%')
            plt.axhline(y=30, color='orange', linestyle='--', alpha=0.6, linewidth=1, label='Minimum: 30%')
            ...
            colors_period = ['#2E8B57' if x > 0 else '#DC143C' for x in period_pnl.values]
            bars = period_pnl.plot(kind='bar', color=colors_period, edgecolor='black', linewidth=1)
            plt.title('⏰ P&L by Trading Period', fontsize=14, fontweight='bold', pad=20)
            plt.ylabel('P&L ($)', fontweight='bold')
            ...
            for i, v in enumerate(period_pnl.values):
                plt.text(i, v + (abs(v) * 0.1 if v != 0 else 1), f'${v:.2f}', 
            ...
            plt.plot(range(len(cumulative_pnl)), cumulative_pnl, linewidth=3, 
                    color='#1f77b4', label='Cumulative P&L')
            
            # Fill area with different colors for positive/negative regions
            plt.fill_between(range(len(cumulative_pnl)), cumulative_pnl, 0, 
                           where=(cumulative_pnl >= 0), color='green', alpha=0.3, label='Profit Zone')
            plt.fill_between(range(len(cumulative_pnl)), cumulative_pnl, 0, 
                           where=(cumulative_pnl < 0), color='red', alpha=0.3, label='Loss Zone')
            
            plt.title('📈 Cumulative P&L Progression', fontsize=14, fontweight='bold', pad=20)
            ...
            plt.annotate(f'Peak: ${cumulative_pnl.iloc[max_point]:.2f}', 
                        xy=(max_point, cumulative_pnl.iloc[max_point]), 
                        xytext=(10, 20), textcoords='offset points',
                        bbox=dict(boxstyle='round,pad=0.3', facecolor='green', alpha=0.7),
                        arrowprops=dict(arrowstyle='->', connectionstyle='arc3,rad=0'))
            
            plt.annotate(f'Trough: ${cumulative_pnl.iloc[min_point]:.2f}', 
            ...
            plt.subplot(4, 3, 6)
            n, bins, patches = plt.hist(df['pnl'], bins=20, alpha=0.8, edgecolor='black', linewidth=1)
            ...
            plt.axvline(x=0, color='black', linestyle='-', linewidth=3, label='Break-even', alpha=0.8)
            plt.axvline(x=df['pnl'].mean(), color='gold', linestyle='--', linewidth=2, 
                       label=f'Mean: ${df["pnl"].mean():.2f}')
            plt.axvline(x=df['pnl'].median(), color='orange', linestyle='--', linewidth=2,
            ...
            plt.subplot(4, 3, 7)
            n, bins, patches = plt.hist(df['return_pct'], bins=20, alpha=0.8, edgecolor='black', linewidth=1)
            ...
            plt.axvline(x=df['return_pct'].mean(), color='gold', linestyle='--', linewidth=2,
            ...
            plt.subplot(4, 3, 8)
            n, bins, patches = plt.hist(df['hold_duration'], bins=15, alpha=0.8, 
                                      color='#20B2AA', edgecolor='black', linewidth=1)
            
            mean_duration = df['hold_duration'].mean()
            plt.axvline(x=mean_duration, color='red', linestyle='-', linewidth=3,
                       label=f'Mean: {mean_duration:.1f} min')
            plt.axvline(x=df['hold_duration'].median(), color='orange', linestyle='--', linewidth=2,
                       label=f'Median: {df["hold_duration"].median():.1f} min')
            
            plt.title('⏱️ Trade Duration Distribution', fontsize=14, fontweight='bold', pad=20)
            ...
            plt.subplot(4, 3, 9)
            period_stats = df.groupby(['period', 'is_profitable']).size().unstack(fill_value=0)
            bars = period_stats.plot(kind='bar', stacked=True, color=['#DC143C', '#2E8B57'], 
                                   edgecolor='black', linewidth=1)
            plt.title('🎯 Win/Loss Distribution by Period', fontsize=14, fontweight='bold', pad=20)
            ...
            for container in bars.containers:
                plt.bar_label(container, label_type='center', fontweight='bold', color='white')
            ...
            for stock in stock_stats.index:
                ...
                plt.scatter(std_return, mean_return, s=size, alpha=0.7, color=color, 
                          edgecolors='black', linewidth=2)
                plt.annotate(stock, (std_return, mean_return), xytext=(8, 8), 
...
Avg Loss: ${metrics['avg_loss']:.2f}
            ...
            period_data = df.groupby('period').agg({
                'is_profitable': 'mean',
                'pnl': 'count'
            }) * [100, 1]  # Convert win rate to percentage
            
            x = np.arange(len(period_data.index))
            width = 0.35
            
            bars1 = plt.bar(x - width/2, period_data['is_profitable'], width, 
                           label='Win Rate (%)', color='#4682B4', alpha=0.8, edgecolor='black')
            
            # Secondary y-axis for trade count
            ax2 = plt.gca().twinx()
            bars2 = ax2.bar(x + width/2, period_data['pnl'], width, 
                           label='Trade Count', color='#FF6347', alpha=0.8, edgecolor='black')
            ...
            plt.gca().set_title('📊 Win Rate & Volume by Period', fontsize=14, fontweight='bold', pad=20)
            
            plt.gca().set_xticks(x)
            plt.gca().set_xticklabels(period_data.index)
            plt.gca().axhline(y=50, color='gold', linestyle='--', alpha=0.8, linewidth=2)
            ...
            chart_path = os.path.join(output_dir, f'comprehensive_pnl_analysis_{timestamp}.png')
    ...
    def generate_detailed_report(self, trades, metrics, output_dir='reports/daily'):
        ...
        try:
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            report_path = os.path.join(output_dir, f'comprehensive_pnl_report_{timestamp}.txt')
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Average P&L per Trade:      ${metrics['avg_pnl_per_trade']:+.2f}\n")
                f.write(f"Total Volume Traded:        ${metrics['total_value_traded']:,.2f}\n")
                f.write(f"Return on Volume:           {(metrics['total_pnl']/metrics['total_value_traded']*100):+.3f}%\n\n")
                ...
                f.write(f"{'Stock':<8} {'Total P&L':<12} {'Trades':<8} {'Avg P&L':<10} {'Win Rate':<10} {'Volume':<12}\n")
                f.write("-" * 70 + "\n")
                ...
                for stock in stock_analysis.index:
                    ...
                    win_rate = stock_analysis.loc[stock, ('is_profitable', 'mean')] * 100
                    volume = stock_analysis.loc[stock, ('trade_value', 'sum')]
                    
                    f.write(f"{stock:<8} ${total_pnl:+9.2f} {trade_count:>7} ${avg_pnl:+8.2f} {win_rate:>8.1f}% ${volume:>10,.0f}\n")
                
                # Determine most/least profitable
                stock_pnl = df.groupby('symbol')['pnl'].sum().sort_values(ascending=False)
                f.write(f"\nMost Profitable Stock:  {stock_pnl.index[0]} (${stock_pnl.iloc[0]:+.2f})\n")
                f.write(f"Least Profitable Stock: {stock_pnl.index[-1]} (${stock_pnl.iloc[-1]:+.2f})\n\n")
                
                # Position Analysis (Long/Short)
                f.write("2. POSITION DIRECTION ANALYSIS\n")
                f.write("-" * 50 + "\n")
                position_analysis = df.groupby('side').agg({
                    'pnl': ['sum', 'count', 'mean'],
                    'is_profitable': 'mean'
                }).round(2)
                ...
                for side in position_analysis.index:
                    ...
                    win_rate = position_analysis.loc[side, ('is_profitable', 'mean')] * 100
                ...
                f.write(f"{'Period':<10} {'Total P&L':<12} {'Trades':<8} {'Avg P&L':<10} {'Win Rate':<10}\n")
                f.write("-" * 60 + "\n")
                ...
                for period in ['Morning', 'Midday', 'Evening']:
                    if period in period_analysis.index:
                        ...
                        win_rate = period_analysis.loc[period, ('is_profitable', 'mean')] * 100
                        
                        f.write(f"{period:<10} ${total_pnl:+9.2f} {trade_count:>7} ${avg_pnl:+8.2f} {win_rate:>8.1f}%\n")
                ...
                f.write(f"Risk-Reward Ratio:          {metrics['risk_reward_ratio']:.2f}\n")
                f.write(f"Expected Value:             ${metrics['expected_value']:+.2f}\n")
                f.write(f"Average P&L per Trade:      ${metrics['avg_pnl_per_trade']:+.2f}\n\n")
                
                f.write("Risk-Adjusted Returns:\n")
                f.write(f"  Average Return:           {metrics['avg_return_pct']:+.3f}%\n")
                f.write(f"  Return Volatility:        {metrics['std_return_pct']:.3f}%\n")
                ...
                f.write("-" * 50 + "\n")
                f.write(f"{'#':<4} {'Stock':<6} {'Qty':<5} {'Buy':<8} {'Sell':<8} {'P&L':<8} {'Return':<8} {'Period':<8}\n")
                f.write("-" * 70 + "\n")
                
                for i, trade in enumerate(trades, 1):
                    f.write(f"{i:<4} {trade['symbol']:<6} {trade['qty']:<5.0f} "
