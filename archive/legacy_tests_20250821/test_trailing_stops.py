...
def test_trailing_stop_system():
    ...
    try:
        ...
        for i, (price, description) in enumerate(price_scenarios, 1):
            ...
            if status:
                trailing_active = (
                    "🚀 ACTIVE" if status["is_trailing_active"] else "⏳ WAITING"
                )
                print(f"  Status: {trailing_active}")
                print(
                    f"  Current Profit: {status['profit_pct']:.1%} (${status['unrealized_pnl']:.2f})"
                )
                ...
                if update_info:
                    print(
                        f"  🔄 STOP UPDATED: ${update_info['old_stop_price']:.2f} → ${update_info['new_stop_price']:.2f}"
                    )
        ...
        if final_status:
            ...
            print(
                f"Final Profit: {final_status['profit_pct']:.1%} (${final_status['unrealized_pnl']:.2f})"
            )

            if final_status["is_trailing_active"]:
                print(f"Protected Profit: {final_status['profit_protected_pct']:.1%}")
                protected_amount = (
                    final_status["profit_protected_pct"] * entry_price * quantity
                )
    ...
    for scenario_name, prices in scenarios:
        ...
        for price in prices[1:]:  # Skip entry price
            ...
            activation_threshold = entry_price * (
                1 + config["TRAILING_STOP_ACTIVATION"]
            )
