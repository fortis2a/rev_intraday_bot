...
def test_confidence_filter():
    ...
    for item in signals:
        signal = item['signal']
        print(f"  {item['strategy']}: {signal['action']} - Confidence: {signal['confidence']:.1%}")
    ...
    for threshold in thresholds:
        ...
        for item in signals:
            ...
            if signal['confidence'] >= threshold:
                passed_signals.append(f"{item['strategy']} ({signal['confidence']:.1%})")
            else:
                filtered_signals.append(f"{item['strategy']} ({signal['confidence']:.1%})")
        
        print(f"{threshold:.1%}          {len(passed_signals):<15} {', '.join(passed_signals) if passed_signals else 'None'}")
        if filtered_signals:
    ...
    live_passed = [item for item in signals if item['signal']['confidence'] >= live_threshold]
    demo_passed = [item for item in signals if item['signal']['confidence'] >= demo_threshold]
    
    print(f"\n[LIVE TRADING MODE] (75% threshold):")
    if live_passed:
        for item in live_passed:
            signal = item['signal']
            print(f"  ✅ {item['strategy']}: {signal['action']} - {signal['confidence']:.1%} - {signal['reason']}")
    else:
