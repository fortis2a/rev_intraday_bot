...
class MultiPagePNGReport:
    ...
    def create_overview_png(self, all_data, output_dir):
        ...
        fig.suptitle('TRADING PERFORMANCE OVERVIEW - ALL DAYS', fontsize=32, fontweight='bold', y=0.95)
        ...
        if all_stock_pnl:
            ...
            plt.title('OVERALL STOCK PERFORMANCE', fontsize=24, fontweight='bold', pad=20)
        ...
        if daily_totals:
            ...
            plt.xticks(range(len(dates)), [d.split('-')[2] + '/' + d.split('-')[1] for d in dates], 
    ...
    def create_daily_png(self, date_str, orders, page_num, output_dir):
        ...
        if date_str == '2025-08-12':
            fig.suptitle(f'{day_name}\\nWARNING: NO PROPER RISK MANAGEMENT', 
                        fontsize=28, fontweight='bold', y=0.95, color='red')
        else:
        ...
        if stock_pnl:
            ...
            for bar, pnl in zip(bars, pnls):
                height = bar.get_height()
                plt.text(bar.get_x() + bar.get_width()/2., height + (10 if height > 0 else -25),
                        f'${pnl:.0f}', ha='center', va='bottom' if height > 0 else 'top', 
                        fontsize=16, fontweight='bold')
...
${worst_stock[1]:+.2f}
        ...
        if stock_pnl:
            ...
            for symbol, pnl in sorted(stock_pnl.items(), key=lambda x: x[1], reverse=True):
                ...
                time_range = f"{min(times)}-{max(times)}" if len(times) > 1 else times[0] if times else 'N/A'
                
                table_data.append([symbol, f'${pnl:+.2f}', trade_count, f'${avg_pnl:+.2f}', time_range, status])
            
            table = plt.table(cellText=table_data,
                             colLabels=['Stock', 'Total P&L', 'Trades', 'Avg P&L', 'Time Range', 'Status'],
                             cellLoc='center',
                             loc='center',
                             bbox=[0, 0, 1, 1])
        ...
        # Save daily page
        daily_path = os.path.join(output_dir, f'page_{page_num:02d}_{date_str.replace("-", "_")}.png')
    ...
    def create_multi_page_pngs(self, output_dir='reports/daily'):
        ...
        try:
            ...
            print(f"Creating multi-page PNG report with {len(all_data)} trading days...")
            ...
            for date_str in sorted(all_data.keys()):
                ...
                daily_path = self.create_daily_png(date_str, all_data[date_str], page_num, output_dir)
