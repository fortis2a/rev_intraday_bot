        ...
        while True:
            ...
            if os.path.exists(engine_log):
                ...
                if current_size > last_engine_size:
                    print(f"✅ Trading Engine: ACTIVE (log growing: {current_size} bytes)")
                    ...
                    if "engine_stale" not in alerts_shown:
                        print(f"🚨 ALERT: Trading Engine STALE ({age_minutes:.0f} min since update)")
                        alerts_shown.add("engine_stale")
                    else:
                        print(f"⚠️  Trading Engine: STALE ({age_minutes:.0f} min)")
                else:
                    print(f"⏳ Trading Engine: Waiting (last update {age_minutes:.1f} min ago)")
            else:
