...
def test_risk_limits():
    ...
    print(f"Risk Check - Daily Loss: {daily_pnl <= -config['MAX_DAILY_LOSS']} (should be False)")
    
    # Check maximum positions
    positions = dm.get_positions()
    print(f"Current Positions: {len(positions)}")
    print(f"Max Positions Limit: {config['MAX_POSITIONS']}")
    print(f"Risk Check - Max Positions: {len(positions) >= config['MAX_POSITIONS']} (should be False)")
    ...
    if not risk_check_passes:
        ...
        if risk_daily_loss:
            print(f"   - Daily loss limit exceeded: ${daily_pnl:.2f} <= -${config['MAX_DAILY_LOSS']}")
        if risk_max_positions:
            print(f"   - Maximum positions limit exceeded: {len(positions)} >= {config['MAX_POSITIONS']}")
    else:
