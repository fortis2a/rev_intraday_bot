...
def test_error_conditions():
    ...
    from stock_specific_config import get_real_time_confidence_for_trade, should_execute_trade
    ...
    try:
        ...
        print(f"SOXL Real-time: {result['confidence']:.1f}% - Tradeable: {result['tradeable']}")
        
        decision = should_execute_trade('SOXL')
        print(f"Trade Decision: Execute={decision['execute']}, Confidence={decision['confidence']:.1f}%")
    ...
    try:
        ...
        print(f"Error Result: Confidence={result['confidence']:.1f}%, Tradeable={result['tradeable']}")
        print(f"Error Mode: {result.get('mode', 'unknown')}")
        print(f"Error Message: {result.get('error', 'No error message')}")
        
        decision = should_execute_trade('SOXL')
        print(f"Trade Decision: Execute={decision['execute']}, Error={decision.get('error', False)}")
    ...
    try:
        ...
        print(f"Invalid Symbol Result: Confidence={result['confidence']:.1f}%, Tradeable={result['tradeable']}")
