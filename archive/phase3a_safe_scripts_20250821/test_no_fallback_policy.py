...
def test_no_fallback_policy():
    ...
    try:
        ...
        print(
            f"✅ Normal operation: {len(filtered)}/{len(test_watchlist)} stocks tradeable"
        )
        ...
        if filtered:
            ...
            print(
                f"✅ Trade decision working: {decision['execute']} for {decision['symbol']}"
            )
    ...
    try:
        ...
        sys.modules["core.real_time_confidence_DISABLED"] = sys.modules.pop(
            "core.real_time_confidence"
        )
        ...
        print(
            f"🛡️ Strict policy enforced: {len(filtered)}/{len(test_watchlist)} stocks tradeable (should be 0)"
        )
        ...
        if test_watchlist:
            ...
            if not decision["execute"] and decision.get("error", False):
                ...
                print(
                    "❌ FAILURE: Trade execution still allowed without real-time data"
                )

        # Restore module
        sys.modules["core.real_time_confidence"] = sys.modules.pop(
            "core.real_time_confidence_DISABLED"
        )
    ...
    try:
        ...
        filtered_historical = get_filtered_watchlist(
            test_watchlist, 75.0, use_real_time=False
        )

        print(
            f"📈 Historical mode: {len(filtered_historical)}/{len(test_watchlist)} stocks would pass"
        )
