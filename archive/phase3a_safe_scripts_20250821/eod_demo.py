...
def create_sample_data():
    ...
    for i in range(50):  # 50 sample trades
        ...
        if np.random.random() < 0.6:  # 60% winners
            ...
            pnl_pct = -np.random.lognormal(0, 0.7) * 0.01  # Larger losses (risk management)
...
def create_demo_charts(df):
    ...
    plt.tight_layout()
    plt.savefig(report_dir / "charts" / "demo_pnl_by_stock.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2. Hourly Trading Activity
    hourly_trades = df.groupby('entry_hour').agg({
        'pnl': ['count', 'sum', 'mean']
    }).round(2)
    ...
    ax1.set_title('Trading Activity by Hour - Demo Data', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(report_dir / "charts" / "demo_hourly_activity.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(report_dir / "charts" / "demo_win_loss_distribution.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    # Trade Timeline
    fig.add_trace(
        go.Scatter(x=df['entry_time'], y=df['pnl'],
                  mode='markers', name='Trade P&L',
                  marker=dict(color=df['pnl'], 
                            colorscale='RdYlGn', 
                            size=10,
    ...
    fig.update_layout(
        title_text="EOD Trading Dashboard - DEMO DATA",
        showlegend=False,
        height=800
    )
