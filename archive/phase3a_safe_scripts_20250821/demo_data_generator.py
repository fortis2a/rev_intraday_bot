...
class DemoDataGenerator:
    def __init__(self):
        ...
        self.base_date = datetime.now().replace(
    ...
    def _calculate_summary(self, trades: List[Dict]) -> Dict[str, Any]:
        ...
        for symbol in set(trade["symbol"] for trade in trades):
            ...
            symbol_summary[symbol] = {
                ...
                "win_rate": (
                    len(symbol_winners) / len(symbol_trades) if symbol_trades else 0
                ),
                "total_pnl": sum(t["pnl"] for t in symbol_trades),
                "avg_pnl_per_trade": (
                    sum(t["pnl"] for t in symbol_trades) / len(symbol_trades)
                    if symbol_trades
                    else 0
                ),
            }
...
def main():
    ...
    print(
        f"📊 Generated {summary['total_trades']} trades for {len(summary['symbol_breakdown'])} symbols"
    )
    ...
    for symbol, stats in summary["symbol_breakdown"].items():
        ...
        print(
            f"   {status} {symbol}: {stats['win_rate']:.1%} win rate, ${stats['total_pnl']:+.2f} P&L ({stats['total_trades']} trades)"
        )
