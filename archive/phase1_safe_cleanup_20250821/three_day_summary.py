...
def generate_three_day_summary():
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        print(
            f"{'Date':<12} {'Trades':<8} {'Symbols':<8} {'Cash Flow':<12} {'Alpaca P&L':<12} {'Status':<15}"
        )
        ...
        for row in rows:
            ...
            print(
                f"{date_str:<12} {trades:<8} {unique_symbols:<8} ${cash_flow:<11.2f} ${alpaca_pnl:<11.2f} {status:<15}"
            )

        print(f"{'-'*12} {'-'*8} {'-'*8} {'-'*12} {'-'*12} {'-'*15}")
        print(
            f"{'TOTAL':<12} {total_trades:<8} {len(all_symbols):<8} ${total_cash_flow:<11.2f} ${total_alpaca_pnl:<11.2f}"
        )
        ...
        for row in rows:
            ...
            print(
                f"   {pnl_icon} {date_str}: {trades} trades, ${alpaca_pnl:.2f} P&L (${avg_per_trade:.2f}/trade)"
            )
            if symbols_traded:
        ...
        print(
            f"   • System automatically uses Alpaca as source of truth when available"
        )
