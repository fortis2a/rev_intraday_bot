...
class IntradayEngine:
    ...
    def __init__(self, mode="LIVE"):
        ...
        if account_info:
            self.logger.info(
                f"[ACCOUNT] Starting Equity: ${account_info['equity']:,.2f}"
            )
            self.logger.info(
    ...
    def recover_existing_positions(self):
        ...
        try:
            ...
            for position in positions:
                ...
                self.logger.info(
                    f"[RECOVERY] {symbol}: {qty} shares @ ${entry_price:.2f}, "
                ...
                if side == "long":
                    ...
                    try:
                        ...
                        thresholds = {
                            ...
                            "trailing_activation_pct": config[
                    ...
                    if current_price:
                        ...
                        if current_price <= stop_loss_price:
                            self.logger.warning(
                                f"[RECOVERY] {symbol} below stop loss ${stop_loss_price:.2f}, "
                            ...
                            self.logger.info(
                                f"[RECOVERY] {symbol} above take profit ${take_profit_price:.2f}, "
                                f"current: ${current_price:.2f} - SELLING"
                            )
                            self.order_manager.place_sell_order(symbol)
                            continue
                    ...
                    try:
                        ...
                        if current_price:
                            self.order_manager.trailing_stop_manager.update_position_price(
                            ...
                            if (
                                current_profit_pct
                                >= thresholds["trailing_activation_pct"]
                            ):
                                self.logger.info(
                                    f"[RECOVERY] {symbol} profit {current_profit_pct:.1%} >= {thresholds['trailing_activation_pct']:.1%}"
                                )
                                self.logger.info(
                                    f"[RECOVERY] {symbol} Trailing stop should be ACTIVE on restart"
                                )

                        self.logger.info(
                            f"[RECOVERY] {symbol} added to trailing stop management"
                        )
                        self.logger.info(
                            f"[RECOVERY] {symbol} Stop: ${stop_loss_price:.2f}, "
                        ...
                        self.logger.error(
                            f"[RECOVERY] Failed to add {symbol} to trailing stop: {e}"
                        )
                    ...
                    if current_price:
                        ...
                        if current_price >= stop_loss_price:
                            self.logger.warning(
                                f"[RECOVERY] {symbol} ABOVE stop loss ${stop_loss_price:.2f}, "
                                f"current: ${current_price:.2f} - COVERING SHORT"
                            )
                            self.order_manager.place_buy_order(
                                symbol, {"strategy": "stop_loss_recovery"}
                            )
                            continue

                        elif current_price <= take_profit_price:
                            self.logger.info(
                                f"[RECOVERY] {symbol} below take profit ${take_profit_price:.2f}, "
                                f"current: ${current_price:.2f} - COVERING SHORT FOR PROFIT"
                            )
                            self.order_manager.place_buy_order(
                                symbol, {"strategy": "profit_target_recovery"}
                            )
                            continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.logger.info(
                                f"[RECOVERY] Stop loss order placed for {symbol} - Order ID: {stop_order.id}"
                            )

                            # Store stop order ID for potential updates
                            if hasattr(
                                self.order_manager.trailing_stop_manager, "stop_orders"
                            ):
                                self.order_manager.trailing_stop_manager.stop_orders[
                                    symbol
                                ] = stop_order.id
                            ...
                            self.logger.error(
                                f"[RECOVERY] Failed to place stop loss for {symbol}: {e}"
                            )
                        ...
                        if current_price:
                            ...
                            if (
                                ...
                                self.logger.info(
                                    f"[RECOVERY] {symbol} SHORT profit {current_profit_pct:.1%} >= {thresholds['trailing_activation_pct']:.1%}"
                                )
                                self.logger.info(
                                    f"[RECOVERY] {symbol} SHORT Trailing stop should be ACTIVE on restart"
                                )

                        self.logger.info(
                            f"[RECOVERY] {symbol} SHORT added to trailing stop management"
                        )
                        self.logger.info(
                            f"[RECOVERY] {symbol} Stop: ${stop_loss_price:.2f} (+{thresholds['stop_loss_pct']:.2%}), "
                        ...
                        self.logger.error(
                            f"[RECOVERY] Failed to add SHORT {symbol} to trailing stop: {e}"
                        )

                else:
                    self.logger.warning(
                        f"[RECOVERY] {symbol} has unknown position side: {side}"
                    )

            if self.active_positions:
                self.logger.info(
                    f"[RECOVERY] Successfully recovered {len(self.active_positions)} positions"
                )
                self.logger.info("=" * 60)
                ...
                for symbol, pos_data in self.active_positions.items():
                    self.logger.info(
                        f"{symbol}: {pos_data['qty']} shares @ ${pos_data['entry_price']:.2f}"
                    )
                    self.logger.info(
                        f"  Stop: ${pos_data['stop_loss']:.2f} | Target: ${pos_data['take_profit']:.2f}"
                    )
                self.logger.info("=" * 60)
    ...
    def check_risk_limits(self):
        ...
        try:
            ...
            if self.daily_pnl <= -config["MAX_DAILY_LOSS"]:
                self.logger.warning(
                    f"[RISK] Daily loss limit reached: ${self.daily_pnl:.2f}"
                )
                return False
            ...
            if len(positions) >= config["MAX_POSITIONS"]:
                self.logger.warning(
                    f"[RISK] Maximum positions limit reached: {len(positions)}"
                )
                return False
    ...
    def scan_for_signals(self):
        ...
        self.logger.info(
            f"🔍 SIGNAL SCAN: Starting scan of {len(config['INTRADAY_WATCHLIST'])} symbols: {config['INTRADAY_WATCHLIST']}"
        )

        for i, symbol in enumerate(config["INTRADAY_WATCHLIST"]):
            try:
                self.logger.info(
                    f"📊 PROCESSING [{i+1}/{len(config['INTRADAY_WATCHLIST'])}]: {symbol}"
                )
                ...
                if df.empty or len(df) < 26:  # Enhanced minimum for MACD calculation
                    self.logger.info(
                        f"⚠️ SKIP {symbol}: Insufficient data ({len(df) if not df.empty else 0} bars)"
                    )
                    continue
                ...
                if (
                    ...
                    self.logger.info(
                        f"⚠️ SKIP {symbol}: Price ${current_price:.2f} outside range [${config['MIN_PRICE']}, ${config['MAX_PRICE']}]"
                    )
                    continue

                self.logger.info(
                    f"✅ DATA OK {symbol}: {len(df)} bars, price ${current_price:.2f}"
                )
                ...
                for strategy_name, strategy_class in self.strategy_classes.items():
                    try:
                        ...
                        if signal:
                            ...
                            self.logger.info(
                                f"🎯 SIGNAL GENERATED [{strategy_name}]: {signal['action']} {symbol} - {signal['reason']}"
                            )
                    except Exception as strategy_error:
                        self.logger.error(
                            f"❌ STRATEGY ERROR [{strategy_name}] for {symbol}: {strategy_error}"
                        )
                        continue

                if strategy_signals_count == 0:
                    self.logger.info(
                        f"📈 NO SIGNALS: {symbol} - no strategies triggered"
                    )
                else:
                    self.logger.info(
                        f"🚀 SIGNALS FOUND: {symbol} generated {strategy_signals_count} signals"
                    )
        ...
        self.logger.info(
            f"📋 SCAN COMPLETE: Found {len(signals)} total signals from {len(config['INTRADAY_WATCHLIST'])} symbols"
        )
        return signals
    ...
    def execute_signal(self, signal):
        ...
        try:
            ...
            if not trade_decision["execute"]:
                self.logger.info(
                    f"[CONFIDENCE BLOCK] {symbol} - {trade_decision['reason']}"
                )
                return False
                ...
                if existing_position:
                    if existing_position["side"] == "long":
                        ...
                        if order_result:
                            ...
                            self.logger.info(
                                f"[EXECUTED] SHORT COVER: {symbol} - Trade #{self.trade_count} | Confidence: {trade_decision['confidence']:.1f}%"
                            )
                            return True
                else:
                    ...
                    if order_result:
                        ...
                        if was_watched:
                            self.logger.info(
                                f"[EXECUTED] PRIORITY BUY: {symbol} (was overbought, now oversold) - Trade #{self.trade_count} | Confidence: {trade_decision['confidence']:.1f}%"
                            )
                            # Remove from watch list
                            del self.stocks_to_watch[symbol]
                        else:
                            self.logger.info(
                                f"[EXECUTED] Buy order for {symbol} - Trade #{self.trade_count} | Confidence: {trade_decision['confidence']:.1f}%"
                            )
                        return True
                ...
                if existing_position:
                    if existing_position["side"] == "long":
                        ...
                        if symbol in self.active_positions:
                            position_age = (
                                datetime.now()
                                - self.active_positions[symbol].get(
                                    "timestamp", datetime.now()
                                )
                            ).total_seconds()
                            if position_age < 60:  # Less than 1 minute old
                                self.logger.info(
                                    f"[SKIP] {symbol} position too new ({position_age:.0f}s) - avoiding immediate reversal"
                                )
                                return False
                        ...
                        if order_result:
                            ...
                            if symbol in self.active_positions:
                                del self.active_positions[symbol]
                            self.logger.info(
                                f"[EXECUTED] Sell order for {symbol} - Trade #{self.trade_count} | Confidence: {trade_decision['confidence']:.1f}%"
                            )
                            return True
                    elif existing_position["side"] == "short":
                    ...
                    if config.get("ENABLE_SHORT_SELLING", False):
                        ...
                        if order_result:
                            ...
                            self.logger.info(
                                f"[EXECUTED] SHORT SELL: {symbol} - Trade #{self.trade_count} | Confidence: {trade_decision['confidence']:.1f}%"
                            )
                            return True
                        else:
                            ...
                            self.logger.info(
                                f"[WATCH] {symbol} short failed - watching for BUY opportunity | Confidence: {trade_decision['confidence']:.1f}%"
                            )
                    else:
                        ...
                        self.logger.info(
                            f"[WATCH] {symbol} overbought (no position) - watching for BUY opportunity | Confidence: {trade_decision['confidence']:.1f}%"
                        )
            ...
            self.logger.error(
                f"[ERROR] Failed to execute signal for {signal['symbol']}: {e}"
            )
            return False
    ...
    def update_pnl(self):
        ...
        try:
            ...
            if positions:
                self.logger.info(
                    f"[PNL] Unrealized P&L: ${total_unrealized:.2f} from {len(positions)} positions"
                )
        ...
        try:
            ...
            if config["TRAILING_STOP_ENABLED"]:
                ...
                if triggered_positions:
                    self.logger.info(
                        f"[TRAILING] {len(triggered_positions)} positions closed by trailing stops"
                    )
            ...
            self.logger.info(
                f"[DEBUG] Signal scan completed - found {len(signals)} signals"
            )
            ...
            self.logger.error(
                f"[DEBUG] Exception in trading_cycle, is_running still {self.is_running}"
            )
    ...
    def run(self):
        ...
        try:
            ...
            while self.is_running:
                self.logger.info(
                    f"[DEBUG] Loop iteration {cycle_count + 1}, is_running={self.is_running}"
                )
                ...
                if self.is_market_hours():
                    ...
                    self.trading_cycle()
                    self.logger.info(
                        f"[DEBUG] trading_cycle() completed, is_running={self.is_running}"
                    )

                    if not self.is_running:
                        self.logger.warning(
                            f"[DEBUG] Engine stopped during trading_cycle - breaking loop"
                        )
                        break

                    # Sleep between cycles
                    self.logger.info(
                        f"[WAIT] Waiting {config['CHECK_INTERVAL']} seconds until next cycle"
                    )
    ...
    def cleanup(self):
        ...
        try:
            ...
            if account_info:
                self.logger.info(
                    f"[FINAL] Final equity: ${account_info['equity']:,.2f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate setup"
    )
