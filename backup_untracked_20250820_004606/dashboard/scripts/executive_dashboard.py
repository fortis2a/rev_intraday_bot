...
class ExecutiveDashboard:
    ...
    def create_executive_dashboard(self, output_dir='reports/daily'):
        ...
        try:
            ...
            fig.suptitle('📊 Executive Trading Dashboard - Daily Performance Summary', 
            ...
            for bar, pnl in zip(bars, pnls):
                ...
                plt.text(bar.get_x() + bar.get_width()/2., height + (abs(height) * 0.1 if height != 0 else 1),
            ...
            plt.pie(trade_counts, labels=symbols, autopct='%1.0f%%', startangle=90,
...
🏦 Account Status:
   ...
   • Return: {(data['account_pnl']/data['equity']*100):+.3f}%
            ...
            plt.text(0.05, 0.95, metrics_text, transform=ax3.transAxes, fontsize=11,
            ...
            # Performance zones
            plt.fill_between(angles, 0, 1, where=(angles <= np.pi/3), color='red', alpha=0.3, label='Poor')
            plt.fill_between(angles, 0, 1, where=((angles > np.pi/3) & (angles <= 2*np.pi/3)), color='yellow', alpha=0.3, label='Average')
            plt.fill_between(angles, 0, 1, where=(angles > 2*np.pi/3), color='green', alpha=0.3, label='Good')
            
            # Calculate performance score (0-100)
            score = max(0, min(100, (data['account_pnl'] + 50) * 2))  # Normalize around break-even
            score_angle = score / 100 * np.pi
            
            # Draw needle
            plt.arrow(0, 0, 0.8*np.cos(score_angle), 0.8*np.sin(score_angle), 
            ...
            plt.text(0, -0.1, f'Score: {score:.0f}/100', ha='center', fontsize=12, fontweight='bold')
            ...
            plt.axis('off')
            plt.title('📋 Stock Performance Matrix', fontsize=14, fontweight='bold', pad=15)
            ...
            plt.plot(x_pos, cumulative_pnl, marker='o', linewidth=3, markersize=8, 
                    color='#1f77b4', markerfacecolor='white', markeredgecolor='#1f77b4', markeredgewidth=2)
            
            # Fill area
            plt.fill_between(x_pos, cumulative_pnl, 0, alpha=0.3, 
                           color='green' if cumulative_pnl[-1] > 0 else 'red')
            
            plt.title('📈 Cumulative Performance', fontsize=14, fontweight='bold', pad=15)
            ...
            if cumulative_pnl:
                plt.annotate(f'Final: ${cumulative_pnl[-1]:.2f}', 
                           xy=(len(symbols)-1, cumulative_pnl[-1]), 
                           xytext=(10, 20), textcoords='offset points',
                           bbox=dict(boxstyle='round,pad=0.3', 
                                   facecolor='green' if cumulative_pnl[-1] > 0 else 'red', alpha=0.7),
            ...
            dashboard_path = os.path.join(output_dir, f'executive_dashboard_{timestamp}.png')
