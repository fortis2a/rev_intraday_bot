    ...
    try:
        ...
        if signal and signal['action'] == 'SELL':
            ...
            if existing_position:
                print(f"⚠️  Existing Position: {existing_position['side']} {existing_position['qty']} shares")
            else:
                ...
                shares = int(min(max_short_position, account_info['equity'] * 0.01) / current_price)
                ...
                if confidence >= min_confidence:
                    ...
                    print(f"   - Stop Loss: ${stop_loss_price:.2f} ({thresholds['stop_loss_pct']*100:.2f}%)")
                    print(f"   - Take Profit: ${take_profit_price:.2f} ({thresholds['take_profit_pct']*100:.2f}%)")
                else:
                    print(f"❌ Confidence Check: {confidence:.1f}% < {min_confidence}%")
