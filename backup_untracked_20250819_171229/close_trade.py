...
class QuickTradeCloser:
    ...
    def list_positions(self):
        ...
        try:
            ...
            print("=" * 70)
            print(
                f"{'#':<3} {'Symbol':<8} {'Side':<6} {'Qty':<6} {'Entry':<10} {'Current':<10} {'P&L $':<12} {'P&L %':<8}"
            )
            print("-" * 70)
            ...
            for i, pos in enumerate(positions, 1):
                ...
                print(
                    f"{status} {i:<2} {symbol:<8} {side:<6} {abs(qty):<6} ${entry_price:<9.2f} ${current_price:<9.2f} ${unrealized_pnl:<11.2f} {pnl_pct:+.1f}%"
                )
    ...
    def close_position(self, symbol):
        ...
        try:
            ...
            print(
                f"📊 Position: {side} {abs_qty} shares @ ${float(position.avg_entry_price):.2f}"
            )
    ...
    def close_all_positions(self):
        ...
        try:
            ...
            for pos in positions:
                ...
                print(
                    f"    • {pos.symbol}: {side} {abs(int(pos.qty))} shares (P&L: ${float(pos.unrealized_pl):.2f})"
                )
                ...
                if hasattr(result, "__iter__"):
                    for order in result:
                        if hasattr(order, "symbol"):
                            print(
                                f"  • {order.symbol}: Order {order.id} ({order.status})"
                            )
