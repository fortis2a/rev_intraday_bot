...
class TradingDashboard:
    ...
    def get_trading_data(self, start_date=None, end_date=None):
        ...
        try:
            ...
            print(f"🔍 Fetching orders from {buffer_start.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} (EOD method)")
            ...
            try:
                ...
                for activity in activities:
                    ...
                    if (hasattr(activity, 'activity_type') and 
                        activity.activity_type.upper() in ['FEE', 'FEES', 'REGULATORY_FEE', 'TAF', 'CAT']):
                        fees_data.append({
                            'date': activity.date,
                            'symbol': getattr(activity, 'symbol', 'N/A'),
                            'fee_amount': float(activity.net_amount),
                            'description': getattr(activity, 'description', activity.activity_type)
                        })
                
                if fees_data:
                    total_fees = sum(fee['fee_amount'] for fee in fees_data)
                    print(f"💸 Found {len(fees_data)} fee entries totaling ${total_fees:.2f}")
                else:
                ...
                try:
                    ...
                    if recent_activities:
                        activity_types = set(getattr(act, 'activity_type', 'UNKNOWN') for act in recent_activities)
                        print(f"🔍 Recent activity types: {activity_types}")
                        # Check if any are fee-related
                        for act in recent_activities[:5]:
                            if hasattr(act, 'activity_type') and 'fee' in act.activity_type.lower():
            ...
            for order in filled_orders:
                orders_data.append({
                    ...
                    'price': float(order.filled_avg_price),  # EOD uses filled_avg_price for price
    ...
    def calculate_pnl_data(self, df, target_start_date=None, target_end_date=None, account_info=None):
        ...
        for symbol in df['symbol'].unique():
            ...
            while buy_idx < len(buys) and sell_idx < len(sells):
                ...
                if target_start_date and target_end_date:
                    ...
                    target_start = target_start_date.date() if hasattr(target_start_date, 'date') else pd.to_datetime(target_start_date).date()
                    target_end = target_end_date.date() if hasattr(target_end_date, 'date') else pd.to_datetime(target_end_date).date()
        ...
        if not result_df.empty:
            ...
            if abs(total_fees_allocated - total_fees) > 0.01:
                print(f"⚠️ Fee allocation mismatch: Expected ${total_fees:.2f}, Allocated ${total_fees_allocated:.2f}")
        else:
        ...
        self.app.layout = dbc.Container([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dbc.ButtonGroup([
                        dbc.Button("Today", id="preset-today", size="sm", outline=True, color="info"),
                        dbc.Button("7D", id="preset-7d", size="sm", outline=True, color="info"),
                        dbc.Button("30D", id="preset-30d", size="sm", outline=True, color="info"),
                    ], size="sm", className="mb-3")
                ], width=3),
                
                dbc.Col([
                    html.Label("Symbol Filter:", style={'fontWeight': 'bold'}),
                    dcc.Dropdown(
                        id='symbol-dropdown',
                        multi=True,
            ...
            dbc.Row([
                dbc.Col([
                    dcc.Graph(
                        ...
                        config={
                            'displayModeBar': True,
                            'toImageButtonOptions': {'format': 'png', 'filename': 'equity_curve'},
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dcc.Graph(id="hourly-performance-chart")
                ], width=6)
            ], className="mb-4"),
            
            dbc.Row([
                dbc.Col([
                    dcc.Graph(id="trade-size-analysis")
                ], width=6),
                dbc.Col([
    ...
    def setup_callbacks(self):
        ...
        def update_dashboard(start_date, end_date, selected_symbols, side_filter, refresh_clicks, intervals):
            ...
            if not pnl_df.empty and account_info and 'day_pnl' in account_info:
                ...
                if trade_pnl_sum != 0:  # Avoid division by zero
                    ...
                    print(f"🔥 CORRECTING P&L: Trade sum ${trade_pnl_sum:.2f} → Account ${account_day_pnl:.2f} (factor: {correction_factor:.4f})")
            ...
            if selected_symbols:
                ...
                filtered_pnl_df = filtered_pnl_df[filtered_pnl_df['symbol'].isin(selected_symbols)]
            
            if side_filter != 'all' and not filtered_pnl_df.empty:
                filtered_pnl_df = filtered_pnl_df[filtered_pnl_df['side'] == side_filter]
            
            # Generate all components with date range context and account info
            metrics_cards = self.create_metrics_cards(filtered_df, filtered_pnl_df, target_start_date, target_end_date, account_info)
            ...
            return (symbol_options, metrics_cards, equity_fig, pnl_dist_fig, 
                   symbol_perf_fig, hourly_fig, trade_size_fig, risk_fig, last_updated)

    def create_metrics_cards(self, df, pnl_df, target_start_date=None, target_end_date=None, account_info=None):
        """Create key metrics cards"""
        if df.empty:
            return html.Div("No trading data available", className="text-center text-muted")
        ...
        if target_start_date and target_end_date:
            ...
            target_orders = df[df['filled_at'].dt.date.between(target_start, target_end)]
            total_orders_in_period = len(target_orders)
            total_volume_in_period = target_orders['trade_value'].sum()
            symbols_traded = len(target_orders['symbol'].unique()) if not target_orders.empty else 0
        else:
        ...
        cards = dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{total_orders_in_period}", className="card-title", style={'color': '#00d4ff'}),
                        html.P("Total Orders", className="card-text"),
                        html.Small(f"(filled in period)", className="text-muted")
                    ])
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{completed_trades}", className="card-title", style={'color': '#17a2b8'}),
                        html.P("Completed Trades", className="card-text"),
                        html.Small(f"(matched pairs)", className="text-muted")
                    ])
                ], className="mb-3", color="dark", outline=True)
            ], width=2),
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"${display_pnl:.2f}", 
                               className="card-title",
                               style={'color': '#28a745' if display_pnl >= 0 else '#dc3545'}),
                ...
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{win_rate:.1f}%", 
                               className="card-title",
                               style={'color': '#28a745' if win_rate >= 50 else '#ffc107'}),
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"${total_volume_in_period:,.0f}", className="card-title", style={'color': '#6c757d'}),
                        html.P("Total Volume", className="card-text"),
                        html.Small(f"(in selected period)", className="text-muted")
                    ])
                ], className="mb-3", color="dark", outline=True)
            ], width=2),
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H4(f"{symbols_traded}", className="card-title", style={'color': '#ffffff'}),
    ...
    def create_equity_curve(self, pnl_df):
        ...
        if pnl_df.empty:
            ...
            print(f"🔥 DEBUG: Creating bar chart with {len(pnl_df)} trades")  # Debug print
            print(f"🔥 DEBUG: Trade P&L sum before equity curve: ${pnl_df['pnl'].sum():.2f}")  # Debug
            ...
            print(f"🔥 DEBUG: Final cumulative P&L: ${cumulative_pnl.iloc[-1]:.2f}")  # Debug
    ...
    def create_symbol_performance(self, pnl_df):
        ...
        print(f"🔥 DEBUG: Symbol P&L DataFrame sum: ${pnl_df['pnl'].sum():.2f}")  # Debug
        fig = go.Figure()
        ...
        if pnl_df.empty:
            ...
            colors = ['#00d4ff', '#28a745', '#ffc107', '#dc3545', '#6f42c1', '#fd7e14', '#20c997', '#e83e8c']
            color_idx = 0
            ...
            for symbol in sorted(pnl_df['symbol'].unique()):
                symbol_data = pnl_df[pnl_df['symbol'] == symbol].sort_values('exit_time')
    ...
    def create_risk_metrics(self, pnl_df):
        ...
        if pnl_df.empty or len(pnl_df) < 2:
            ...
            metrics = ['Sharpe Ratio', 'Sortino Ratio', 'Max Drawdown ($)', 'VaR 95% ($)']
            ...
            fig.add_trace(go.Bar(
                ...
                y=values,
                marker_color=colors,
                text=[f'{v:.3f}' if 'Ratio' in m else f'${v:.2f}' for m, v in zip(metrics, values)],
                textposition='auto'
            ))
