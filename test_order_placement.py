...
def test_order_placement():
    ...
    for symbol in test_symbols:
        ...
        print(f"Confidence Decision: Execute={decision['execute']}, Confidence={decision['confidence']:.1f}%")
        ...
        existing = next((p for p in positions if p['symbol'] == symbol and p['side'] == 'long'), None)
        if existing:
            print(f"BLOCKED: {symbol} - Already have position: {existing['qty']} shares")
            continue
        ...
        shares = order_manager.calculate_position_size(symbol, current_price, account_info['equity'])
        ...
        print(f"⚠️  WOULD PLACE BUY ORDER FOR {symbol} (debug mode - not actually placing)")
