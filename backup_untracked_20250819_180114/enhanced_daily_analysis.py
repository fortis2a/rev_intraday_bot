...
class EnhancedDailyAnalysis:
    ...
    def match_cross_day_trades(self, today_orders, yesterday_orders):
        ...
        all_symbols = set(
            list(yesterday_by_symbol.keys()) + list(today_by_symbol.keys())
        )
    ...
    def analyze_cross_day_performance(self, cross_day_trades, today_only_trades):
        ...
        if cross_day_trades:
            print(
                f"📅 Cross-Day Trades (opened yesterday, closed today): {len(cross_day_trades)}"
            )
            print(
                f"{'Symbol':<8} {'Open':<12} {'Close':<12} {'P&L $':<12} {'P&L %':<10}"
            )
            print("-" * 60)
            ...
            for trade in cross_day_trades:
                ...
                print(
                    f"{status} {symbol:<7} ${open_price:<11.2f} ${close_price:<11.2f} ${pnl:<11.2f} {pnl_pct:+.1f}%"
                )
        ...
        if today_only_trades:
            ...
            for trade in today_only_trades:
                ...
                status = "🟢" if pnl >= 0 else "🔴"
                print(
                    f"{status} {symbol:<7} ${buy_price:<11.2f} ${sell_price:<11.2f} ${pnl:<11.2f} {pnl_pct:+.1f}%"
                )
        ...
        try:
            ...
            if portfolio_history and hasattr(portfolio_history, "equity"):
                ...
                if len(equity_values) > 1:
                    ...
                    day_pnl_pct = (
                        (day_pnl / start_value) * 100 if start_value > 0 else 0
                    )
        ...
        try:
            ...
            if positions:
                ...
                for pos in positions:
                    ...
                    status = "🟢" if pnl >= 0 else "🔴"
                    print(
                        f"{status} {pos.symbol:<7} {side:<6} {qty:<8.0f} ${abs(value):<11.2f} ${pnl:+.2f}"
                    )
    ...
    def generate_enhanced_summary(self):
        ...
        cross_day_trades, today_only_trades = self.match_cross_day_trades(
            today_orders, yesterday_orders
        )

        # Analyze performance including cross-day trades
        total_realized_pnl = self.analyze_cross_day_performance(
            cross_day_trades, today_only_trades
        )
