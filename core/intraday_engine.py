...
class IntradayEngine:
    ...
    def __init__(self, demo_mode=False, bypass_market_hours=False):
        ...
        self.timeframe_config = (
            config  # Use main config instead of separate timeframe config
        )
        ...
        if not getattr(self.data_manager, "api", None):
            self.logger.error(
                "❌ CRITICAL: Alpaca live data connection not established during engine init"
            )
            self.logger.error(
                "🔎 Troubleshoot: 1) Verify ALPACA_API_KEY / ALPACA_SECRET_KEY 2) Check internet access 3) Confirm alpaca-py installed 4) Paper account not rate-limited"
            )
            raise RuntimeError(
                "Alpaca live data connection required – initialization aborted."
            )
        ...
        try:
            if hasattr(self.order_manager, "initialize_trading"):
                if not self.order_manager.initialize_trading():
                    self.logger.warning(
                        "⚠️ OrderManager trading connection failed - running in simulation mode"
                    )
        ...
        self.failed_signal_cooldown_period = getattr(
            config, "FAILED_SIGNAL_COOLDOWN_SECONDS", 300
        )
        ...
        self.logger.info(
            f"✅ Scalping engine initialized - Timeframe: {config.TIMEFRAME}"
        )
        # Reporting state
    ...
    def get_diagnostics(self) -> Dict[str, object]:
        ...
        if hasattr(self, "last_signal_check"):
            try:
                last_check_age = round(
                    self._get_timestamp_age_seconds(self.last_signal_check), 1
                )
            except Exception:
                last_check_age = None
        ...
        try:
            if hasattr(self, "order_manager") and hasattr(
                self.order_manager, "get_last_error"
            ):
        ...
        return {
            ...
            "global_pause_s": (
                max(0, int((getattr(self, "global_pause_until", 0) or 0) - now))
                if getattr(self, "global_pause_until", 0) > now
                else 0
            ),
    ...
    def can_submit_order(self, symbol: str) -> bool:
        ...
        if current_time - last_order < self.min_order_interval:
            ...
            self.logger.debug(
                f"⏳ Wash trade prevention: waiting {remaining_time:.1f}s more for {symbol}"
            )
            return False
    ...
    def record_failed_signal(self, symbol: str):
        ...
        self.logger.info(
            f"🚫 Recording failed signal for {symbol} - extended cooldown activated"
        )
    ...
    def validate_signal_price(self, signal, current_market_data) -> bool:
        ...
        if not current_market_data:
            self.logger.warning(
                f"⚠️ No current market data to validate signal for {signal.symbol}"
            )
            return False
        ...
        if signal_age > 30:  # 30 seconds max age
            self.logger.warning(
                f"🚫 Signal rejected for {signal.symbol}: Signal too old ({signal_age:.1f}s)"
            )
            return False
        ...
        if not fresh_price:
            self.logger.warning(
                f"⚠️ Could not get fresh market data for {signal.symbol}"
            )
            return False
        ...
        if data_age > 10:  # 10 seconds max data age
            self.logger.warning(
                f"🚫 Signal rejected for {signal.symbol}: Market data too stale ({data_age:.1f}s)"
            )
            return False

        # Calculate price gap percentage
        price_gap_pct = (
            abs(current_price - signal_entry) / signal_entry * 100
            if signal_entry
            else 999
        )
        ...
        if price_gap_pct > max_price_gap:
            ...
            self.logger.info(
                f"Data consistency: Signal age {signal_age:.1f}s, Data age {data_age:.1f}s"
            )
            return False

        self.logger.debug(
            f"✅ Signal validated for {signal.symbol}: Gap {price_gap_pct:.2f}% (Max: {max_price_gap:.2f}%)"
        )
        return True
    ...
    def _get_dynamic_price_gap_tolerance(self, symbol: str, price: float) -> float:
        ...
        final_tolerance = (
            base_tolerance * symbol_multiplier * price_multiplier * time_multiplier
        )
        ...
        if (
            symbol not in self._last_tolerance_log
            or abs(self._last_tolerance_log[symbol] - final_tolerance) > 0.1
        ):
            self.logger.debug(
                f"📊 {symbol} tolerance: {final_tolerance:.1f}% "
        ...
        try:
            ...
            for symbol in config.INTRADAY_WATCHLIST[:10]:  # Limit to 10 for demo
                try:
                    ...
                    if (
                        not config.ADOPT_EXISTING_POSITIONS
                        and hasattr(self, "_excluded_symbols")
                        and symbol in self._excluded_symbols
                    ):
                        continue
                    ...
                    try:
                        ...
                        if symbol in bars and len(bars[symbol]) > 0:
                            ...
                            volume = (
                                500000  # Assume sufficient volume if data unavailable
                            )
                        ...
                        self.logger.debug(
                            f"Volume check error for {symbol}: {volume_err}"
                        )
                    ...
                    if not (config.MIN_PRICE <= current_price <= config.MAX_PRICE):
                        reasons.append(
                            f"price {current_price:.2f} outside [{config.MIN_PRICE},{config.MAX_PRICE}]"
                        )
                    if volume < config.MIN_VOLUME:
                        reasons.append(f"vol {volume} < {config.MIN_VOLUME}")
                    if spread_pct > config.MAX_SPREAD_PCT:
                        reasons.append(
                            f"spread {spread_pct:.3f}% > {config.MAX_SPREAD_PCT}%"
                        )
                    if not reasons:
                        filtered_symbols.append(symbol)
                        self.logger.debug(
                            f"✅ {symbol} passed filters: ${current_price:.2f}, Vol: {volume:,}"
                        )
                    else:
            ...
            current_time = time.time()
            if (
                current_time - self.last_watchlist_log_time >= 300
            ):  # Log every 5 minutes
                self.logger.info(
                    f"📊 Filtered watchlist: {len(filtered_symbols)} symbols from {len(config.INTRADAY_WATCHLIST)}"
                )
        ...
        try:
            ...
            try:
                ...
                if time.time() - last_diag >= 60:
                    ...
                    self.logger.info(
                        f"🧪 {symbol} data snapshot: bars={len(data)}, cols={cols[-12:]} (showing last 12)"
                    )
                    required_cols = [
                        "ema_5",
                        "ema_13",
                    ...
                    if missing:
                        ...
                        if nan_cols:
                            self.logger.warning(
                                f"⚠️ {symbol} NaN indicators latest bar: {nan_cols}"
                            )
            ...
            if not current_price:
                self.logger.debug(
                    f"⚠️ No current price data for pre-validation - skipping {symbol}"
                )
            ...
            for strategy_name, strategy_class in self.strategy_classes.items():
                try:
                    ...
                    if strategy_signal:
                        ...
                        self.logger.info(
                            f"🧬 {strategy_name} raw signals {len(strategy_signals)} for {symbol}"
                        )
                        total_strategy_raw += len(strategy_signals)
                        for sig in strategy_signals:
                            sig.timestamp = generation_start_time
                            price_gap = (
                                abs(sig.entry_price - current_price)
                                / current_price
                                * 100
                                if current_price
                                else 999
                            ...
                            max_gap = (
                                0.75
                                if symbol in {"IONQ", "RGTI", "QBTS", "QUBT"}
                                else 0.5
                            )
                    ...
                    self.logger.warning(
                        f"❌ Strategy {strategy_name} error {symbol}: {strat_err}"
                    )
                    continue

            if signals:
                current_short_exposure = getattr(
                    self.risk_manager, "total_short_exposure", 0
                )
                max_short_exposure = config.MAX_SHORT_EXPOSURE
                ratio = (
                    current_short_exposure / max_short_exposure
                    if max_short_exposure
                    else 0
                )
                if ratio > 0.8:
                    for sig in signals:
                ...
                best = signals[0]
                self.logger.info(
                    f"🎯 Best signal {symbol} {best.signal_type} {best.strategy} conf={best.confidence:.2f}"
                )
                return [best]
            else:
                if not hasattr(self, "signal_rejections"):
                ...
                self.logger.info(
                    f"🛑 No executable signals {symbol} raw={total_strategy_raw} gap_rej={price_gap_rejections}"
                )
                return []
        except Exception as e:
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    broker_position = self.order_manager.get_position_info(
                        symbol, context="order_fill_verification", force_fresh=True
                    )

                    if (
                        broker_position
                        and abs(float(broker_position.get("qty", 0))) > 0
                    ):
                        # Normalize side detection: prefer explicit 'side' (long/short), then qty sign, then market_value
                        broker_side = self._normalize_broker_position_side(
                            broker_position
                        )
                        ...
                        if broker_side == intended_side:
                            ...
                            qty_tolerance = max(
                                1, expected_quantity * 0.1
                            )  # 10% tolerance or 1 share minimum

                            if qty_difference <= qty_tolerance:
                                self.logger.info(
                                    f"✅ ORDER FILL VERIFIED: {symbol} - {broker_side} {broker_qty} shares"
                                )
                                return True
                            else:
                                self.logger.warning(
                                    f"⚠️ Quantity mismatch: expected {expected_quantity}, got {broker_qty}"
                                )
                        else:
                            self.logger.warning(
                                f"⚠️ Direction mismatch: expected {intended_side}, got {broker_side}"
                            )
                        ...
                        try:
                            ...
                            if order_status:
                                ...
                                if status in ["filled", "partially_filled"]:
                                    ...
                                    if filled_qty > 0:
                                        self.logger.info(
                                            f"✅ ORDER STATUS VERIFIED: {raw_order_id} filled {filled_qty} shares"
                                        )
                                        return True
                                elif status in ["cancelled", "rejected", "expired"]:
                                    self.logger.warning(
                                        f"🚫 ORDER CANCELLED/REJECTED: {raw_order_id} status: {status}"
                                    )
                                    return False
                        except Exception as status_error:
                            self.logger.debug(
                                f"Could not check order status: {status_error}"
                            )
                    ...
                    self.logger.warning(
                        f"Error in fill verification check: {check_error}"
                    )
                    time.sleep(check_interval)

            # Timeout reached without confirmation
            self.logger.error(
                f"⏰ ORDER FILL VERIFICATION TIMEOUT: {raw_order_id} after {timeout}s"
            )
            self.logger.error(
                f"🚫 Cannot confirm order fill - will not create position tracking"
            )

            # Final check: see if position exists now
            try:
                final_position = self.order_manager.get_position_info(
                    symbol, context="final_verification", force_fresh=True
                )
                if final_position and abs(float(final_position.get("qty", 0))) > 0:
                    self.logger.warning(
                        f"⚠️ Position exists after timeout - possible delayed fill"
                    )
                    return True
    ...
    def sync_positions_with_broker(self):
        ...
        try:
            if (
                ...
                or not hasattr(self.order_manager, "data_manager")
                or not self.order_manager.data_manager.api
            ):
                return
            ...
            if not actual_positions:
                ...
                if self.active_positions:
                    self.logger.warning(
                        f"🧹 Clearing {len(self.active_positions)} orphaned internal positions"
                    )
            ...
            actual_symbols = {
                pos["symbol"] for pos in actual_positions if abs(float(pos["qty"])) > 0
            }
            ...
            if untracked_positions:
                ...
                if new_untracked:
                    if config.ADOPT_EXISTING_POSITIONS:
                        ...
                        self.logger.info(
                            f"[ADOPT] Adopting {len(new_untracked)} existing positions for management: {new_untracked}"
                        )
                        ...
                        for symbol in new_untracked:
                            ...
                            if position_data:
                                ...
                                self.logger.debug(
                                    f"🔍 Position data fields for {symbol}: {list(position_data.keys())}"
                                )
                                ...
                                if "avg_entry_price" in position_data:
                                    ...
                                    self.logger.warning(
                                        f"⚠️ Cannot determine entry price for {symbol}, skipping adoption"
                                    )
                                    continue

                                # Calculate current price
                                current_price = (
                                    float(position_data["market_value"]) / abs(qty)
                                    if qty != 0
                                    else entry_price
                                )
                                ...
                                profit_target = (
                                    entry_price * (1 + config.PROFIT_TARGET_PCT / 100)
                                    if signal_type == "BUY"
                                    else entry_price
                                    * (1 - config.PROFIT_TARGET_PCT / 100)
                                )
                                stop_loss = (
                                ...
                                self.logger.info(
                                    f"✅ Adopted {symbol}: {signal_type} {abs(qty)} shares @ ${entry_price:.2f}"
                                )
                                self.logger.info(
                                    f"   📊 Current: ${current_price:.2f} | P&L: ${unrealized_pnl:+.2f} ({pnl_pct:+.2f}%)"
                                )
                                self.logger.info(
                                    f"   🎯 Targets: Profit ${profit_target:.2f} | Stop ${stop_loss:.2f}"
                                )
                        ...
                        self.logger.warning(
                            f"🚨 Found {len(new_untracked)} positions not in internal tracking: {new_untracked}"
                        )
                        self.logger.warning(
                            f"💡 These may be from partial fills or order timing - will be excluded from new trades"
                        )
                        self._untracked_positions_warned.update(new_untracked)
                        ...
                        for symbol in untracked_positions:
                            if symbol not in getattr(self, "_excluded_symbols", set()):
                                ...
                                self.logger.debug(
                                    f"⛔ Excluding {symbol} from trading (untracked position)"
                                )
            ...
            if orphaned_tracking:
                self.logger.warning(
                    f"🧹 Removing {len(orphaned_tracking)} orphaned position tracking: {orphaned_tracking}"
                )
    ...
    def execute_signal(self, signal: ScalpingSignal) -> bool:
        ...
        try:
            self.logger.error(
                f"🔥 EXECUTE_SIGNAL ENTRY: {signal.symbol} {signal.signal_type} @ ${signal.entry_price:.2f}"
            )
            self.logger.info(
                f"📋 STARTING execute_signal for {signal.symbol} {signal.signal_type} @ ${signal.entry_price:.2f}"
            )
            ...
            try:
                ...
                confidence_decision = should_execute_trade(
                    signal.symbol, signal.signal_type
                )

                if not confidence_decision["execute"]:
                    self.logger.warning(
                        f"🚫 CONFIDENCE REJECTED: {signal.symbol} - {confidence_decision['reason']}"
                    )
                    self.logger.info(
                        f"📋 FAILED: Individualized confidence check failed for {signal.symbol}"
                    )
                    return False
                else:
                    self.logger.info(
                        f"✅ CONFIDENCE APPROVED: {signal.symbol} - {confidence_decision['confidence']:.1f}%"
                    )
                ...
                self.logger.error(
                    f"❌ Confidence system error for {signal.symbol}: {conf_error}"
                )
                self.logger.info(
                    f"📋 FAILED: Confidence system error for {signal.symbol}"
                )
                return False
            ...
            if not self._pre_trade_filter(signal):
                self.logger.debug(
                    f"🚫 Pre-trade filter rejected {signal.symbol} ({signal.signal_type}) conf={getattr(signal, 'confidence', None)}"
                )
                self.logger.info(
                    f"📋 FAILED: Pre-trade filter rejected {signal.symbol}"
                )
                return False
            ...
            if signal.symbol in self.recently_closed_profitable:
                ...
                if age < self.profitable_closure_cooldown:
                    ...
                    self.logger.info(
                        f"🛡️ SKIPPING {signal.symbol} - profitable closure cooldown active ({remaining_cooldown:.0f}s remaining)"
                    )
                    self.logger.info(
                        f"📋 FAILED: Profitable closure cooldown for {signal.symbol}"
                    )
                    return False
                else:
                    ...
                    self.logger.info(
                        f"✅ {signal.symbol} cooldown expired - signals allowed again"
                    )

            self.logger.info(
                f"📋 CHECKPOINT 1: Cooldown checks passed for {signal.symbol}"
            )

            # Check actual broker position first to prevent accumulation
            actual_position = self.order_manager.get_position_info(
                signal.symbol, context="signal_execution", force_fresh=True
            )
            ...
            if actual_position and abs(float(actual_position.get("qty", 0))) > 0:
                ...
                if (existing_side == "buy" and signal_side == "sell") or (
                    ...
                    self.logger.info(
                        f"[CLOSE] Closing existing {existing_side} position due to opposite {signal_side} signal for {signal.symbol}"
                    )
                    return self.close_position(signal.symbol)
                else:
                    # Same direction signal - reject to prevent accumulation
                    self.logger.warning(
                        f"[WARNING] REJECTING {signal_side} signal for {signal.symbol} - already have {existing_side} position"
                    )
                    self.logger.info(
                        f"📋 FAILED: Position conflict for {signal.symbol}"
                    )
                    return False

            self.logger.info(
                f"📋 CHECKPOINT 2: Position checks passed for {signal.symbol}"
            )
            ...
            if signal.symbol in self.active_positions:
                ...
                if (existing_signal_type == "BUY" and signal.signal_type == "SELL") or (
                    ...
                    self.logger.info(
                        f"[CLOSE] Closing tracked {existing_signal_type} position due to opposite {signal.signal_type} signal for {signal.symbol}"
                    )
                    return self.close_position(signal.symbol)
                else:
                    self.logger.debug(
                        f"[WARNING] Ignoring {signal.signal_type} signal for {signal.symbol} - already have {existing_signal_type} position"
                    )
                    self.logger.info(
                        f"[FAILED] FAILED: Tracked position conflict for {signal.symbol}"
                    )
                    return False

            self.logger.info(
                f"📋 CHECKPOINT 3: Tracked position checks passed for {signal.symbol}"
            )
            ...
            try:
                if not self.risk_manager.can_open_position(
                    ...
                    self.logger.warning(
                        f"❌ Risk check failed for {signal.symbol} ({signal.signal_type}) @ ${signal.entry_price:.2f}"
                    )
                    self.logger.info(
                        f"📋 FAILED: Risk check failed for {signal.symbol}"
                    )
                    return False
            except Exception as risk_exception:
                # CRITICAL FIX: Handle risk limit exceptions properly
                self.logger.error(
                    f"🚫 RISK LIMIT VIOLATION for {signal.symbol}: {risk_exception}"
                )
                self.logger.error(
                    f"🛑 ORDER CANCELLED - Will NOT create phantom position"
                )
            ...
            try:
                ...
                base_stop_pct = (
                    stock_thresholds["stop_loss_pct"] * 100
                )  # Convert to percentage
                profit_target_pct = (
                    stock_thresholds["take_profit_pct"] * 100
                )  # Convert to percentage
                self.logger.info(
                    f"📊 STOCK-SPECIFIC THRESHOLDS {signal.symbol}: Stop {base_stop_pct:.2f}% | Profit {profit_target_pct:.2f}%"
                )
            except Exception as threshold_error:
                self.logger.warning(
                    f"⚠️ Could not get stock-specific thresholds for {signal.symbol}: {threshold_error}"
                )
                base_stop_pct = config.STOP_LOSS_PCT
                profit_target_pct = config.PROFIT_TARGET_PCT
                self.logger.info(
                    f"📊 FALLBACK THRESHOLDS {signal.symbol}: Stop {base_stop_pct:.2f}% | Profit {profit_target_pct:.2f}%"
                )
            ...
            if getattr(config, "USE_ATR_STOPS", False):
                try:
                    bars = self.data_manager.get_bars(
                        signal.symbol, timeframe=config.TIMEFRAME, limit=100
                    )
                    if (
                        bars is not None
                        and len(bars) >= config.ATR_PERIOD + 2
                        and all(c in bars.columns for c in ["high", "low", "close"])
                    ):
                        highs = bars["high"]
                        lows = bars["low"]
                        closes = bars["close"]
                        ...
                        if len(trs) >= config.ATR_PERIOD:
                            ...
                            if atr_pct and atr_pct > 0:
                                adaptive_stop_pct = max(
                                    base_stop_pct,
                                    config.ATR_MULT * atr_pct,
                                    config.MIN_STOP_PCT,
                                )
                                adaptive_stop_pct = min(
                                    adaptive_stop_pct, config.MAX_STOP_PCT
                                )
                except Exception as atr_err:
                    self.logger.debug(
                        f"ATR sizing calc error {signal.symbol}: {atr_err}"
                    )
            # Build a provisional stop price for sizing
            provisional_stop = (
                entry_price * (1 - adaptive_stop_pct / 100)
                if signal.signal_type == "BUY"
                else entry_price * (1 + adaptive_stop_pct / 100)
            )
            signal.stop_loss = provisional_stop
            signal.profit_target = (
                entry_price * (1 + profit_target_pct / 100)
                if signal.signal_type == "BUY"
                else entry_price * (1 - profit_target_pct / 100)
            )
            ...
            if position_size <= 0:
                self.logger.warning(
                    f"❌ Invalid position size for {signal.symbol} ({signal.signal_type}). entry=${signal.entry_price:.2f} stop=${signal.stop_loss:.2f} calc_size={position_size}"
                )
                self.logger.info(
                    f"📋 FAILED: Invalid position size for {signal.symbol}"
                )
                return False

            self.logger.info(
                f"📋 CHECKPOINT 5: Position sizing passed for {signal.symbol} (size={position_size})"
            )

            # Check wash trade prevention
            if not self.can_submit_order(signal.symbol):
                self.logger.debug(
                    f"⏳ Delaying order for {signal.symbol} - wash trade prevention"
                )
                self.logger.info(
                    f"📋 FAILED: Wash trade prevention for {signal.symbol}"
                )
                return False

            self.logger.info(
                f"📋 CHECKPOINT 6: Wash trade checks passed for {signal.symbol}"
            )

            # CRITICAL: Clear any pending orders before opening new position
            self.logger.info(
                f"🧹 Clearing pending orders for {signal.symbol} before new position"
            )
            cancelled_count = self.order_manager.cancel_pending_orders_for_symbol(
                signal.symbol
            )
            if cancelled_count > 0:
                self.logger.info(
                    f"✅ Cancelled {cancelled_count} pending orders for {signal.symbol}"
                )
            ...
            self.logger.info(
                f"🎯 SUBMITTING NEW POSITION: {signal.signal_type} {position_size} shares of {signal.symbol} @ ${signal.entry_price:.2f}"
            )
            self.logger.info(f"🔍 ORDER VALIDATION: Intended side = {intended_side}")
            self.logger.info(
                f"📋 CHECKPOINT 7: About to submit order for {signal.symbol}"
            )
            ...
            order_result = self.order_manager.submit_market_order(
                symbol=signal.symbol, side=intended_side, quantity=position_size
            )

            self.logger.info(
                f"📋 CHECKPOINT 8: Order submission result for {signal.symbol}: {bool(order_result)}"
            )
            ...
            if order_result:
                ...
                if str(readable_id).startswith("SIM_"):
                    ...
                    self.logger.warning(
                        f"⚠️ Position tracking may not reflect real broker state"
                    )

                # NEW: Verify order fill before creating position tracking
                fill_info = self._verify_order_fill(
                    order_result, signal.symbol, intended_side, position_size
                )

                if not fill_info:
                    self.logger.error(
                        f"🚫 ORDER NOT FILLED: {readable_id} for {signal.symbol}"
                    )
                ...
                try:
                    last_err = getattr(
                        self.order_manager, "get_last_error", lambda: None
                    )()
                except Exception:
                    last_err = None
                if last_err:
                    ...
                    det = last_err.get("details")
                    self.logger.error(
                        f"🧩 EXECUTION FAILURE for {signal.symbol} ({signal.signal_type}) | ERROR_CODE={code} MSG={msg} DETAILS={det}"
                    )
                else:
                    self.logger.error(
                        f"🧩 EXECUTION FAILURE for {signal.symbol} ({signal.signal_type}) | ERROR_CODE=unknown MSG=no_details"
                    )
                ...
                actual_position = self.order_manager.get_position_info(
                    signal.symbol, context="execution_price", force_fresh=True
                )
                if actual_position and abs(float(actual_position.get("qty", 0))) > 0:
                    # Calculate actual average entry price from broker
                    actual_entry_price = abs(
                        float(
                            actual_position.get("avg_entry_price", signal.entry_price)
                        )
                    )
                    actual_qty = abs(float(actual_position.get("qty", position_size)))

                    self.logger.info(
                        f"📍 ACTUAL EXECUTION: {signal.symbol} - {actual_qty} shares @ ${actual_entry_price:.2f}"
                    )
                    ...
                    self.logger.info(
                        f"🎯 RECALCULATED LEVELS: {signal.symbol} - Stop: ${actual_stop_loss:.2f}, Target: ${actual_profit_target:.2f}"
                    )
                    ...
                    self.logger.warning(
                        f"⚠️ Could not get actual execution price for {signal.symbol}, using signal prices"
                    )
                ...
                self.risk_manager.track_position_opened(
                    signal.symbol,
                    signal.signal_type,
                ...
                try:
                    ...
                    try:
                        ...
                        bars = self.data_manager.get_bars(
                            signal.symbol, timeframe=config.TIMEFRAME, limit=50
                        )
                        if bars is not None and len(bars) > 20:
                            indicator_result = (
                                unified_indicator_service.get_indicators_for_strategy(
                                    bars, signal.symbol, signal.strategy
                                )
                            )
                            indicators_snapshot = (
                                indicator_result.get("current_values", {})
                                if "error" not in indicator_result
                                else {}
                            )
                        else:
                            indicators_snapshot = {}
                        ...
                        confidence_data = get_real_time_confidence_for_trade(
                            signal.symbol
                        )
                        ...
                        risk_assessment = {
                            ...
                            "max_risk_amount": abs(
                                execution_entry_price - execution_stop_loss
                            )
                            * execution_position_size,
                        }
                        ...
                        self.logger.debug(
                            f"Decision context capture failed {signal.symbol}: {context_error}"
                        )
                        indicators_snapshot = {}
                    ...
                    tr = TradeRecord(
                        ...
                        signal_reason=getattr(
                            signal, "reason", "Signal reason not captured"
                        ),
                    ...
                    self.logger.info(
                        f"   🎯 Reason: {getattr(signal, 'reason', 'Not captured')}"
                    )
                    self.logger.info(f"   📈 Market Regime: {market_regime}")
                    self.logger.info(
                        f"   ⚖️ Risk: {adaptive_stop_pct:.2f}% stop | ATR: {atr_pct or 'N/A'}"
                    )
                    ...
                    self.logger.debug(
                        f"TradeRecord create failed {signal.symbol}: {terr}"
                    )
                return True
    ...
    def manage_positions(self):
        ...
        for symbol, position in self.active_positions.items():
            try:
                ...
                self.update_trailing_stop_with_peak_tracking(
                ...
                planned_stop_pct = position.get(
                    "adaptive_stop_pct", config.STOP_LOSS_PCT
                )
                ...
                if loss_pct >= catastrophic_threshold:
                    ...
                    if loss_pct > 0:
                        self.logger.debug(
                            f"🛡️ Grace active {symbol}: {int(position['stop_grace_until']-now_ts)}s left (loss {loss_pct:.3f}%)"
                        )

                if stop_loss_exceeded and actual_loss_pct > planned_stop_pct * 1.05:
                    overrun = actual_loss_pct - planned_stop_pct
                    self.logger.warning(
                        f"🚨 STOP OVERRUN {symbol}: planned {planned_stop_pct:.3f}% actual {actual_loss_pct:.3f}% (+{overrun:.3f}%)"
                    )
                ...
                if not should_exit:
                    ...
                    # 2. Profit target hit
                        ...
                        signal.signal_type == "BUY"
                        and current_price >= signal.profit_target
                    ):
                        should_exit = True
                ...
                if not should_exit and not grace_active:
                    stop_pct = (
                        planned_stop_pct if planned_stop_pct else config.STOP_LOSS_PCT
                    )
                    if stop_pct > 0:
                        r_mult = pnl_pct / stop_pct
                        position["r_multiple_peak"] = max(
                            position.get("r_multiple_peak", 0.0), r_mult
                        )
                        # Breakeven
                        if not position.get("breakeven_set") and r_mult >= getattr(
                            config, "BREAKEVEN_TRIGGER_R", 1.0
                        ):
                            position["stop_loss"] = position["entry_price"]
                            position["breakeven_set"] = True
                            self.logger.info(
                                f"⚖️ Breakeven set {symbol} at ${position['stop_loss']:.2f} (R={r_mult:.2f})"
                            )
                        # Trailing start
                        if not position.get("trailing_started") and r_mult >= getattr(
                            config, "TRAIL_TRIGGER_R", 1.5
                        ):
                            position["trailing_started"] = True
                            self.logger.info(
                                f"🚀 Trailing enabled {symbol} (R={r_mult:.2f})"
                            )
                        # Apply adaptive trailing if started
                        if position.get("trailing_started"):
                            try:
                                bars = self.data_manager.get_bars(
                                    symbol, timeframe=config.TIMEFRAME, limit=100
                                )
                                lookback = getattr(config, "TRAIL_LOOKBACK", 5)
                                if (
                                    bars is not None
                                    and len(bars) >= lookback + 2
                                    and all(
                                        c in bars.columns
                                    ...
                                    offset = 0.0
                                    if atr_val:
                                        offset = atr_val * getattr(
                                            config, "TRAIL_ATR_OFFSET_MULT", 0.5
                                        )
                                    if signal.signal_type == "BUY":
                                        new_stop = max(
                                            position["stop_loss"], recent_low - offset
                                        )
                                        if new_stop > position["stop_loss"]:
                                            self.logger.info(
                                                f"🔧 Trail raise {symbol}: {position['stop_loss']:.2f} -> {new_stop:.2f}"
                                            )
                                            position["stop_loss"] = new_stop
                                    else:
                                        new_stop = min(
                                            position["stop_loss"], recent_high + offset
                                        )
                                        if new_stop < position["stop_loss"]:
                                            self.logger.info(
                                                f"🔧 Trail lower {symbol}: {position['stop_loss']:.2f} -> {new_stop:.2f}"
                                            )
                            except Exception as trail_err:
                                self.logger.debug(
                                    f"Trail manage error {symbol}: {trail_err}"
                                )
                ...
                if should_exit:
                    ...
                    self.logger.info(
                        f"🚪 Closing {symbol} position: {exit_reason} (P&L: ${unrealized_pnl:+.2f}){peak_info}"
                    )
    ...
    def update_trailing_stop_with_peak_tracking(
        self, symbol: str, position: Dict, current_price: float, current_pnl_pct: float
    ):
        ...
        try:
            ...
            if signal.signal_type == "BUY":
                current_absolute_pnl = (current_price - entry_price) * position[
                    "position_size"
                ]
                is_profitable = current_price > entry_price
                price_movement_favorable = current_price > peak_data["peak_price"]
            else:  # SELL
                current_absolute_pnl = (entry_price - current_price) * position[
                    "position_size"
                ]
                is_profitable = current_price < entry_price
                price_movement_favorable = current_price < peak_data["peak_price"]
            ...
            if price_movement_favorable and current_pnl_pct > peak_data["peak_pnl_pct"]:
                ...
                if (
                    current_pnl_pct >= profit_activation_threshold
                    and not peak_data["trailing_active"]
                ):
                    peak_data["trailing_active"] = True
                    self.logger.info(
                        f"📈 {symbol} trailing stop ACTIVATED at +{current_pnl_pct:.2f}% profit (${current_price:.2f})"
                    )
                    ...
                    if signal.signal_type == "BUY":
                        ...
                        if new_trailing_stop > position["stop_loss"]:
                            ...
                            protected_profit_pct = (
                                (new_trailing_stop - entry_price) / entry_price
                            ) * 100

                            self.logger.info(
                                f"📈 {symbol} trailing stop updated: ${old_stop:.2f} → ${new_trailing_stop:.2f} "
                        ...
                        if new_trailing_stop < position["stop_loss"]:
                            ...
                            # Calculate what percentage this protects
                            protected_profit_pct = (
                                (entry_price - new_trailing_stop) / entry_price
                            ) * 100
            ...
            if current_pnl_pct > 0 and current_pnl_pct >= peak_data["peak_pnl_pct"]:
                if current_pnl_pct >= 0.50:  # 50% higher than profit target
                    self.logger.info(
                        f"🚀 {symbol} exceptional profit: +{current_pnl_pct:.2f}% (${current_price:.2f})"
                    )
                elif current_pnl_pct >= 0.30:  # At profit target
                    self.logger.info(
                        f"🎯 {symbol} profit target reached: +{current_pnl_pct:.2f}% (${current_price:.2f})"
                    )
    ...
    def close_position(self, symbol: str) -> bool:
        ...
        try:
            if symbol not in self.active_positions:
                self.logger.warning(
                    f"⚠️ Cannot close {symbol} - no tracked position exists"
                )
                return False
            ...
            try:
                ...
                if intended_side not in ("buy", "sell"):
                    self.logger.warning(
                        f"⚠️ Intended side unknown for {symbol} during close; proceeding defensively"
                    )
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"🚫 Intended: {intended_side}, Broker(normalized): {broker_side}, Broker(raw side): {dbg_side}, qty: {dbg_qty}, mkt_val: {dbg_mv}"
                        )
                        self.logger.error(f"🚫 This indicates an execution failure")
                    except Exception:
                        self.logger.error(
                            f"🚫 POSITION DIRECTION MISMATCH: {symbol} (intended={intended_side}, broker={broker_side})"
                        )

            except Exception as validation_error:
                self.logger.warning(
                    f"⚠️ Could not validate broker position for {symbol}: {validation_error}"
                )

            # CRITICAL: Cancel any pending orders for this symbol first
            self.logger.info(
                f"🧹 Clearing pending orders for {symbol} before position close"
            )
            cancelled_count = self.order_manager.cancel_pending_orders_for_symbol(
                symbol
            )
            if cancelled_count > 0:
                self.logger.info(
                    f"✅ Cancelled {cancelled_count} pending orders for {symbol}"
                )
                # Wait for cancellations to process
                time.sleep(2)
            ...
            if not self.can_submit_order(symbol):
                self.logger.debug(
                    f"⏳ Delaying position close for {symbol} - wash trade prevention"
                )
                return False
            ...
            self.logger.info(
                f"📤 Submitting {exit_side} order to close {symbol} position"
            )
            exit_order_id = self.order_manager.submit_market_order(
                symbol=symbol, side=exit_side, quantity=position["position_size"]
            )
            ...
            if exit_order_id:
                ...
                try:
                    ...
                    if (
                        hasattr(self.order_manager, "alpaca_trader")
                        and self.order_manager.alpaca_trader
                    ):
                        status = self.order_manager.alpaca_trader.get_order_status(
                            raw_id
                        )
                    ...
                    self.logger.debug(
                        f"Exit fill price lookup failed {symbol}: {_ex_stat}"
                    )
                ...
                self.logger.info(
                    f"📉 Position closed: {symbol} - {position['position_size']} shares @ ${exit_price:.2f}"
                )
                self.logger.info(
                    f"💰 Realized P&L: ${realized_pnl:+.2f} | Daily P&L: ${self.daily_pnl:+.2f}"
                )
                ...
                try:
                    ...
                    if tr:
                        ...
                        try:
                            ...
                            self.logger.debug(
                                f"Symbol perf accumulation failed {symbol}: {acc_err}"
                            )
                ...
                if symbol in self.position_peaks:
                    ...
                    if peak_info["peak_pnl_pct"] > 0:
                        self.logger.info(
                            f"📊 {symbol} peak profit was +{peak_info['peak_pnl_pct']:.2f}% at ${peak_info['peak_price']:.2f}"
                        )
                        ...
                        if peak_info["trailing_active"] and realized_pnl > 0:
                            ...
                            self.logger.info(
                                f"🛡️ {symbol} added to profitable closure cooldown ({self.profitable_closure_cooldown}s)"
                            )
    ...
    def update_performance_metrics(self, pnl: float):
        ...
        if pnl > 0:
            ...
            if self.adaptive_cooldown_multiplier > 1.0 and pnl > 0.01:
                ...
                self.adaptive_cooldown_multiplier = max(
                    1.0, self.adaptive_cooldown_multiplier * self.cooldown_decay_factor
                )
                if int(old_mult * 100) != int(self.adaptive_cooldown_multiplier * 100):
                    self.logger.info(
                        f"🧊 Cooldown multiplier decayed: {old_mult:.2f} -> {self.adaptive_cooldown_multiplier:.2f}"
                    )
        else:
            ...
            if (
                est_risk_per_trade > 0
                and loss_amount
                > est_risk_per_trade * self.significant_loss_threshold_pct
            ):
                old_mult = self.adaptive_cooldown_multiplier
                self.adaptive_cooldown_multiplier = min(
                    self.max_adaptive_cooldown_multiplier,
                    self.adaptive_cooldown_multiplier * self.cooldown_increase_factor,
                )
                if self.adaptive_cooldown_multiplier > old_mult:
                    self.logger.warning(
                        f"🔥 Increasing cooldown multiplier {old_mult:.2f} -> {self.adaptive_cooldown_multiplier:.2f} after loss ${pnl:.2f}"
                    )
            # Extra increase if streak of 3+ consecutive losses
            if len(self.recent_losses) >= 3 and all(
                l > 0 for l in self.recent_losses[-3:]
            ):
                old_mult = self.adaptive_cooldown_multiplier
                self.adaptive_cooldown_multiplier = min(
                    self.max_adaptive_cooldown_multiplier,
                    self.adaptive_cooldown_multiplier * 1.1,
                )
                if self.adaptive_cooldown_multiplier > old_mult:
                    self.logger.warning(
                        f"📉 Losing streak detected - cooldown multiplier {old_mult:.2f} -> {self.adaptive_cooldown_multiplier:.2f}"
                    )

            # Global pause after N consecutive losses
            try:
                if self.consecutive_losses >= getattr(
                    config, "CONSECUTIVE_LOSS_PAUSE_N", 3
                ):
                    pause_sec = max(
                        0,
                        int(getattr(config, "CONSECUTIVE_LOSS_PAUSE_MINUTES", 10)) * 60,
                    )
                    self.global_pause_until = time.time() + pause_sec
                    self.logger.warning(
                        f"⏸️ Global trading pause for {pause_sec//60}m after {self.consecutive_losses} consecutive losses"
                    )
                    self.consecutive_losses = 0
            except Exception:
                pass
        ...
        if self.performance_metrics["total_trades"] % 10 == 0:
            avg_slip = (
                (self.slippage_stats["total"] / self.slippage_stats["count"])
                if self.slippage_stats["count"]
                else 0.0
            )
            self.logger.info(
    ...
    def _record_slippage(
        self, intended_price: float, executed_price: float, side: str, symbol: str
    ):
        ...
        if abs_slip_pct > config.MAX_SLIPPAGE_PCT:
            self.logger.warning(
                f"⚠️ Slippage {abs_slip_pct:.4f}% on {symbol} ({side.upper()}) intended ${intended_price:.2f} executed ${executed_price:.2f}"
            )
        else:
            self.logger.debug(
                f"🪙 Slippage {abs_slip_pct:.4f}% on {symbol} ({side.upper()})"
            )
    ...
    def check_daily_limits(self) -> bool:
        ...
        if self.daily_pnl <= -max_daily_loss_dollar:
            self.logger.warning(
                f"❌ Daily loss limit reached: ${self.daily_pnl:.2f} (>{config.MAX_DAILY_LOSS_PCT}% of ${account_equity:,.2f})"
            )
            return False
        return True
    ...
    def _pre_trade_filter(self, signal: ScalpingSignal) -> bool:
        ...
        try:
            ...
            if self.performance_metrics.get("total_trades", 0) >= getattr(
                config, "MAX_TRADES_PER_DAY", 50
            ):
                self.logger.info(
                    f"🧱 Rejected {signal.symbol}: MAX_TRADES_PER_DAY reached"
                )
                return False

            if self.symbol_trade_count.get(signal.symbol, 0) >= getattr(
                config, "MAX_TRADES_PER_SYMBOL_PER_DAY", 6
            ):
                self.logger.info(
                    f"🧱 Rejected {signal.symbol}: per-symbol trade cap reached"
                )
                return False

            # Enhanced per-symbol spacing for better setups
            spacing = max(
                0, int(getattr(config, "MINUTES_BETWEEN_TRADES_PER_SYMBOL", 0)) * 60
            )
            if spacing:
                ...
                if last_time and (time.time() - last_time) < spacing:
                    ...
                    self.logger.debug(
                        f"🕒 Reject {signal.symbol}: spacing {remain}s remaining"
                    )
                    return False
            ...
            self.logger.info(
                f"🧪 CHECK {signal.symbol}: confidence {signal.confidence:.2f} vs min {min_conf:.2f}"
            )
            if signal.confidence < min_conf:
                self.logger.info(
                    f"🧪 REJECT {signal.symbol}: confidence {signal.confidence:.2f} < {min_conf:.2f}"
                )
                return False
            ...
            if md:
                ...
                max_spread = config.MAX_SPREAD_PCT * (
                    0.7 if self.adaptive_cooldown_multiplier > 1.2 else 0.8
                )
                self.logger.info(
                    f"🧪 CHECK {signal.symbol}: spread {spread:.4f}% vs max {max_spread:.4f}%"
                )
                if spread > max_spread:
                    self.logger.info(
                        f"🧪 REJECT {signal.symbol}: spread {spread:.4f}% > {max_spread:.4f}%"
                    )
                    return False
                ...
                self.logger.info(
                    f"🧪 CHECK {signal.symbol}: volume_ratio {vol_ratio:.2f} vs min {min_vol_ratio:.2f}"
                )
                if vol_ratio < min_vol_ratio:
                    self.logger.info(
                        f"🧪 REJECT {signal.symbol}: volume_ratio {vol_ratio:.2f} < {min_vol_ratio:.2f}"
                    )
                    return False
                ...
                if price <= 0:
                    self.logger.info(
                        f"🧪 REJECT {signal.symbol}: invalid price {price}"
                    )
                    return False
            ...
            try:
                ...
                self.logger.info(
                    f"🧪 CHECK {signal.symbol}: expected_R {expected_r:.2f} vs min {min_expected_r:.2f}"
                )
                if expected_r < min_expected_r:
                    self.logger.info(
                        f"🧪 REJECT {signal.symbol}: expected_R {expected_r:.2f} < {min_expected_r:.2f}"
                    )
                    return False
                ...
                if (
                    risk_pct > 0.5
                ):  # More than 0.5% risk per trade is too much for scalping
                    self.logger.info(
                        f"🧪 REJECT {signal.symbol}: risk_pct {risk_pct:.3f}% > 0.5%"
                    )
                    return False

            except Exception as e:
                self.logger.warning(
                    f"🧪 Risk/reward calculation failed for {signal.symbol}: {e}"
                )
                return False
            ...
            with open(self._trade_log_path, "a", newline="") as tf:
                ...
                writer.writerow(
                    [
                        ...
                        (
                            ""
                            if not self.active_positions.get(tr.symbol)
                            else f"{self.active_positions[tr.symbol].get('adaptive_stop_pct','')}"
                        ),
                        (
                            ""
                            if not self.active_positions.get(tr.symbol)
                                ...
                                ""
                                if self.active_positions[tr.symbol].get("atr_pct_entry")
                                is None
                                else f"{self.active_positions[tr.symbol].get('atr_pct_entry'):.4f}"
                            )
                        ),
                        (
                            ""
                            if not self.active_positions.get(tr.symbol)
                            else f"{self.active_positions[tr.symbol].get('r_multiple_peak','')}"
                        ),
                    ]
                )
        except Exception as e:
    ...
    def run_trading_cycle(self):
        ...
        try:
            ...
            if not self.data_manager.ensure_connection():
                self.logger.error(
                    "[ERROR] CRITICAL: No live data connection after retry - stopping cycle"
                )
                time.sleep(5)
                return
            ...
            try:
                test_symbol = (
                    config.INTRADAY_WATCHLIST[0] if config.INTRADAY_WATCHLIST else "SPY"
                )
                test_price = self.data_manager.get_current_price(test_symbol)
                if test_price is None or test_price <= 0:
                    self.logger.warning(
                        f"⚠️ Cannot get live data for {test_symbol} - continuing anyway"
                    )
                ...
                self.logger.warning(
                    f"⚠️ Market data connection test failed for {test_symbol}: {e}"
                )
            ...
            self.logger.info(
                f"✅ Trading cycle checks passed - Processing {len(config.INTRADAY_WATCHLIST)} symbols"
            )
            ...
            self.logger.info(
                f"📋 WATCHLIST DEBUG: Original {len(config.INTRADAY_WATCHLIST)} symbols: {config.INTRADAY_WATCHLIST}"
            )
            self.logger.info(
                f"📋 FILTERED RESULT: {len(symbols)} symbols passed filters: {symbols}"
            )
            ...
            for symbol in symbols:
                try:
                    self.logger.info(
                        f"🔍 SYMBOL PROCESSING: {symbol} - Starting analysis..."
                    )

                    # Skip if we already have a position in this symbol
                    if symbol in self.active_positions:
                        self.logger.info(
                            f"⏭️ SKIP REASON: {symbol} - already have position in active_positions"
                        )
                        continue
                    ...
                    if (
                        actual_position
                        and abs(float(actual_position.get("qty", 0))) > 0
                    ):
                        self.logger.info(
                            f"⚠️ Skipping {symbol} - has actual position: {actual_position.get('qty', 0)} shares"
                        )
                        continue
                    ...
                    if cooldown_active:
                        self.logger.debug(
                            f"⏳ Cooldown active pre-generation {symbol} - generation allowed for diagnostics"
                        )
                    ...
                    if (
                        data is None or len(data) < 20
                    ):  # Need minimum data for indicators
                        self.logger.info(
                            f"⚠️ Insufficient live data for {symbol} - skipping"
                        )
                        continue
                    ...
                    if hasattr(data, "index") and len(data) > 0:
                        ...
                        try:
                            ...
                            bar_ts = latest_bar_time.to_pydatetime().replace(
                                tzinfo=None
                            )
                            data_age = (datetime.utcnow() - bar_ts).total_seconds()
                        self.logger.info(
                            f"🕒 {symbol} latest bar: {bar_ts} | now: {now_dt} | age: {data_age:.0f}s | last3: {last_times}"
                        )
                        ...
                        if data_age > max_age:
                            if allow_stale:
                                self.logger.warning(
                                    f"⚠️ Using stale data for diagnostics {symbol} (age {data_age:.0f}s > {max_age}s)"
                                )
                            else:
                                self.logger.info(
                                    f"⚠️ Market data too stale for {symbol} ({data_age:.0f}s old)"
                                )
                                continue
                    ...
                    self.logger.info(
                        f"📈 {symbol}: Generated {len(signals) if signals else 0} signals in {signal_generation_time:.2f}s"
                    )

                    # Track signal generation speed
                    if signal_generation_time > 1.0:  # Log slow signal generation
                        self.logger.warning(
                            f"⚠️ Slow signal generation for {symbol}: {signal_generation_time:.2f}s"
                        )
                    ...
                    if (
                        ...
                        and len(self.active_positions) < config.MAX_OPEN_POSITIONS
                    ):
                        self.logger.info(
                            f"🚀 Found {len(signals)} signals for {symbol}, attempting to execute best one..."
                        )
                        ...
                        signal_age = self._get_timestamp_age_seconds(
                            getattr(best_signal, "timestamp", None)
                        )

                        if (
                            signal_age < 5.0
                        ):  # Signal must be less than 5 seconds old for execution
                            try:
                                self.logger.info(
                                    f"💫 Executing {symbol} signal: {best_signal.signal_type} @ ${best_signal.entry_price:.2f}"
                                )
                                ...
                                if execution_success:
                                    self.logger.info(
                                        f"⚡ FAST EXECUTION: {symbol} signal executed in {execution_time:.2f}s"
                                    )
                                else:
                                    self.logger.warning(
                                        f"❌ Signal execution failed for {symbol}"
                                    )
                                ...
                                self.logger.warning(
                                    f"[ERROR] Signal execution failed for {symbol}"
                                )

                        else:
                            self.logger.warning(
                                f"🚫 Signal rejected for {symbol}: Too slow ({signal_age:.1f}s old)"
                            )
                        ...
                        if not signals:
                            ...
                            if cooldown_active:
                                self.logger.info(
                                    f"⏳ Cooldown blocked execution for {symbol} (signals={len(signals)})"
                                )
                            elif (
                                len(self.active_positions) >= config.MAX_OPEN_POSITIONS
                            ):
                                self.logger.info(
                                    f"📈 Max positions reached ({len(self.active_positions)}/{config.MAX_OPEN_POSITIONS}), skipping {symbol}"
                                )
                    ...
                    self.logger.error(
                        f"❌ Error processing {symbol}: {e} | {type(e).__name__}\n{_tb.format_exc()}"
                    )
                    continue
    ...
    def log_status(self):
        ...
        if len(self.active_positions) > 0:
            pos_str = ", ".join(
                [
                    f"{sym}:{pos['signal'].signal_type}"
                    for sym, pos in self.active_positions.items()
                ]
            )
            self.logger.debug(
                f"🕐 {current_time} | Positions: {pos_str} | P&L: ${self.daily_pnl:+.2f} | Trades: {self.performance_metrics['total_trades']} | Win: {win_rate:.1f}%"
            )
        else:
            self.logger.debug(
                f"🕐 {current_time} | Monitoring | P&L: ${self.daily_pnl:+.2f} | Trades: {self.performance_metrics['total_trades']}"
            )
    ...
    def start(self):
        ...
        try:
            if hasattr(self.order_manager, "initialize_trading"):
                if not self.order_manager.initialize_trading():
                    self.logger.warning(
                        "⚠️ OrderManager trading connection failed - using existing connection"
                    )
            else:
                self.logger.info("[OK] Using existing data manager connection")
        except Exception as e:
            self.logger.warning(
                f"[WARNING] OrderManager initialization check failed: {e} - using existing connection"
            )
        ...
        try:
            while self.is_running:
                ...
                self.logger.info(
                    f"🕐 Market hours check: {market_open} (Current time: {datetime.now().strftime('%H:%M:%S')})"
                )
                ...
                if market_open:
                    ...
                    if (
                        current_time - self.last_signal_check
                        >= self.timeframe_config.signal_delay
                    ):
                        self.logger.info(
                            f"🔄 Signal check interval reached, running trading cycle..."
                        )
                        self.run_trading_cycle()
                        self.last_signal_check = current_time
                    else:
                        time_remaining = self.timeframe_config.signal_delay - (
                            current_time - self.last_signal_check
                        )
                        self.logger.info(
                            f"⏳ Next signal check in {time_remaining:.1f}s"
                        )
                    ...
                    self.logger.info(
                        f"❌ NOT in market hours - Current time: {datetime.now().strftime('%H:%M:%S')}, "
                    ...
                    if self.active_positions:
                        self.logger.info(
                            f"⏰ Outside market hours - monitoring {len(self.active_positions)} existing positions..."
                        )
                        ...
                        self.logger.info(
                            "⏰ Outside market hours, no positions to monitor..."
                        )
                        ...
                        try:
                            ...
                            if self._daily_report_generated_date != today:
                                ...
                                if self.trade_count > 0:
                                    self.logger.info(
                                        "📝 Daily report functionality simplified after cleanup"
                                    )
                                    self.logger.info(
                            ...
                            self.logger.warning(
                                f"⚠️ Auto-report generation failed: {rep_e}"
                            )
    ...
    def check_position_stop_losses(self):
        ...
        if not self.data_manager.ensure_connection():
            self.logger.error(
                "❌ CRITICAL: No live data for stop loss checks (after retry) - pausing"
            )
            time.sleep(5)
            return
        ...
        for symbol in list(self.active_positions.keys()):
            try:
                ...
                market_data = self.data_manager.get_current_market_data(
                    symbol, "stop_loss_check"
                )
                if market_data is None:
                    self.logger.warning(
                        f"⚠️ No live data for stop loss check on {symbol}"
                    )
                    continue
                ...
                if market_data.get("source") != "alpaca_live":
                    self.logger.error(
                        f"❌ CRITICAL: Non-live data in stop loss check for {symbol}"
                    )
                    continue
                ...
                if time_since_entry < min_hold_time:
                    ...
                    if loss_pct > 2.0:  # Emergency stop at 2% during hold period
                        self.logger.warning(
                            f"🚨 EMERGENCY STOP during hold period: {symbol} - {loss_pct:.3f}% loss"
                        )
                        self.logger.info(
                            f"🛑 EMERGENCY STOP TRIGGERED: {symbol} - Loss exceeds 2% emergency threshold"
                        )
                        self.close_position(symbol)
                    else:
                        # Skip normal stop loss evaluation during minimum hold time
                        self.logger.debug(
                            f"⏳ {symbol} in minimum hold period ({time_since_entry:.0f}s/{min_hold_time}s) - skipping stop loss"
                        )
                        continue
                ...
                if stop_loss_hit or loss_pct > 0.25:  # Hard stop at 0.25%
                    ...
                    if stop_loss_hit:
                        ...
                        if position["stop_loss"] != original_stop:
                            ...
                            self.logger.info(
                                f"🎯 TRAILING STOP PROTECTED PROFIT: {symbol} - Stop moved from ${original_stop:.2f} to ${position['stop_loss']:.2f}"
                            )
                        else:
                    ...
                    if loss_pct > config.STOP_LOSS_PCT:
                        ...
                        self.logger.warning(
                            f"🚨 RAPID STOP VIOLATION: {symbol} - Expected {config.STOP_LOSS_PCT:.3f}% but lost {loss_pct:.3f}% (excess: {violation:.3f}%)"
                        )

                    # Immediately close position
                    self.logger.info(
                        f"🛑 RAPID STOP TRIGGERED: {symbol} - {exit_reason}"
                    )
                    self.close_position(symbol)

            except Exception as e:
                self.logger.error(
                    f"❌ Error in rapid stop loss check for {symbol}: {e}"
                )
                continue
        ...
        try:
            if (
                self.order_manager
                and hasattr(self.order_manager, "data_manager")
                and self.order_manager.data_manager.api
            ):
                ...
                for pos in broker_positions:
                    ...
                    if loss_pct > 0.25:  # Hard stop at 0.25%
                        self.logger.warning(
                            f"🚨 UNTRACKED POSITION HARD STOP: {symbol} - {loss_pct:.3f}% loss"
                        )
                        ...
                        self.order_manager.submit_market_order(
                            symbol, side, int(abs_qty)
                        )

                    elif loss_pct > config.STOP_LOSS_PCT:
                        self.logger.warning(
                            f"🚨 UNTRACKED POSITION STOP LOSS: {symbol} - {loss_pct:.3f}% loss exceeds {config.STOP_LOSS_PCT}%"
                        )
    ...
    def log_final_report(self):
        ...
        self.logger.info(
            f"Average Trade: ${metrics['total_pnl']/max(metrics['total_trades'], 1):+.2f}"
        )
    ...
    def _accumulate_symbol_performance(self, tr: TradeRecord):
        ...
        sym = tr.symbol
        sp = self.symbol_perf.setdefault(
            sym,
            {
                "trades": 0,
                "wins": 0,
        ...
        cumulative = (
            sp["equity_path"][-1] + tr.realized_pnl
            if sp["equity_path"]
            else tr.realized_pnl or 0.0
        )
    ...
    def get_realtime_symbol_pnl(self) -> List[Dict[str, float]]:
        ...
        for symbol, pos in self.active_positions.items():
            try:
                ...
                risk_per_share = (
                    abs(entry - pos["stop_loss"]) if pos.get("stop_loss") else None
                )
                current_r = (
                    ((price - entry) * direction / risk_per_share)
                    if risk_per_share
                    else None
                )
                snapshot.append(
    ...
    def generate_daily_reports(self):
        ...
        with summary_csv.open("w", newline="") as f:
            ...
            for sym, sp in self.symbol_perf.items():
                ...
                avg_mae_r = (
                    (sum(sp["mae_r_list"]) / len(sp["mae_r_list"]))
                    if sp["mae_r_list"]
                    else ""
                )
                avg_mfe_r = (
                    (sum(sp["mfe_r_list"]) / len(sp["mfe_r_list"]))
                    if sp["mfe_r_list"]
                    else ""
                )
                ...
                w.writerow(
                    [
                        ...
                        f"{avg_r:.3f}",
                        f"{avg_hold:.1f}",
                        f"{sp['max_drawdown']:.2f}",
                        "" if avg_mae_r == "" else f"{avg_mae_r:.3f}",
                        "" if avg_mfe_r == "" else f"{avg_mfe_r:.3f}",
                        sp["missed_gt1R"],
                        f"{avg_unreal:.3f}",
                    ]
                )
        ...
        try:
            with md_report.open("w") as mf:
                ...
                for sym, sp in self.symbol_perf.items():
                    ...
                    mf.write(f"### {sym}\n")
                    mf.write(
                        f"Trades: {sp['trades']} | Win%: {win_rate:.1f}% | Net PnL: {sp['realized_pnl']:.2f} | Avg R: {avg_r:.2f} | Max DD: {sp['max_drawdown']:.2f}\n\n"
                    )
                mf.write("## Files\n\n")
    ...
    def validate_environment(self):
        ...
        try:
            ...
            if self.data_manager.api:
                ...
                if account_info:
                    self.logger.info(
                        f"API connected - Account equity: ${account_info['equity']:,.2f}"
                    )
                    return True
                else:
