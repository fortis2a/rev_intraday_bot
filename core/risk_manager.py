...
class RiskManager:
    ...
    def __init__(self):
        ...
        self.logger.info(f"💰 Max position size: ${config.get('MAX_POSITION_SIZE', 5000)}")
        ...
        try:
            ...
            for symbol, qty in positions.items():
                if qty > 0:
                    ...
                    try:
                        current_price = order_manager.data_manager.get_current_price(symbol)
                        if current_price:
                            exposure = qty * current_price
                            self.total_long_exposure += exposure
                    except Exception as e:
                        self.logger.warning(f"Could not calculate exposure for {symbol}: {e}")
            ...
            self.logger.info(f"📊 Synced with broker - Positions: {self.open_positions_count}, "
    ...
    def can_open_position(self, symbol: str, entry_price: float, signal_type: str) -> bool:
        ...
        try:
            ...
            if self.open_positions_count >= max_positions:
                self.logger.warning(f"[{symbol}] Position limit reached ({self.open_positions_count}/{max_positions})")
                return False
            ...
            if self.daily_pnl <= -daily_loss_limit:
                self.logger.warning(f"[{symbol}] Daily loss limit reached (${self.daily_pnl:.2f})")
                return False
            
            # Check total exposure limit
            max_total_exposure = self.account_equity * config.get('MAX_TOTAL_EXPOSURE_PCT', 0.8)
            current_exposure = self.total_long_exposure + self.total_short_exposure
            
            position_value = self.calculate_position_size(symbol, entry_price, self.account_equity) * entry_price
            
            if current_exposure + position_value > max_total_exposure:
                self.logger.warning(f"[{symbol}] Total exposure limit would be exceeded")
                return False
    ...
    def calculate_position_size(self, symbol: str, price: float, account_equity: float) -> int:
        ...
        try:
            ...
            self.logger.debug(f"[{symbol}] Position size: {shares} shares (${shares * price:.2f})")
            return shares
    ...
    def track_position_opened(self, symbol: str, signal_type: str, quantity: int, entry_price: float):
        ...
        try:
            ...
            self.logger.info(f"[{symbol}] Position opened - {signal_type.upper()} {quantity} @ ${entry_price:.2f}")
            self.logger.info(f"📊 Total positions: {self.open_positions_count}, "
                           f"Long exposure: ${self.total_long_exposure:.2f}")
            
        except Exception as e:
            self.logger.error(f"[{symbol}] Error tracking position opened: {e}")
    
    def track_position_closed(self, symbol: str, quantity: int, exit_price: float, pnl: float):
        ...
        try:
            if symbol in self.positions_tracker:
                ...
                # Update exposure
                if signal_type in ['buy', 'long']:
                    self.total_long_exposure = max(0, self.total_long_exposure - position_value)
                else:
                    self.total_short_exposure = max(0, self.total_short_exposure - position_value)
                ...
                self.logger.info(f"[{symbol}] Position closed - {quantity} @ ${exit_price:.2f}, "
                               f"P&L: ${pnl:.2f}")
                self.logger.info(f"📊 Total positions: {self.open_positions_count}, "
    ...
    def get_risk_summary(self) -> Dict:
        ...
        return {
            ...
            'exposure_pct': (self.total_long_exposure + self.total_short_exposure) / self.account_equity if self.account_equity > 0 else 0,
