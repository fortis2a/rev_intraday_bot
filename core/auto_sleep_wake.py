...
class AutoMarketSleepWake:
    ...
    def display_market_closed_screen(self):
        ...
        if next_open:
            try:
                ...
                if time_diff.total_seconds() > 0:
                    ...
                    print(
                        f"[COUNTDOWN] Time Until Wake: {hours:02d}:{minutes:02d}:{seconds:02d}"
                    )
                    ...
                    if next_open_et.date() > current_et.date():
                        if next_open_et.weekday() == 0:  # Monday
                            ...
                            print(
                                f"[INFO] Next trading day is {next_open_et.strftime('%A')}"
                            )
        ...
        self.logger.info(
            f"[SLEEP] Market closed - Bot sleeping until {next_open_et.strftime('%Y-%m-%d %H:%M:%S %Z') if next_open else 'unknown'}"
        )
        ...
        if next_close:
            try:
                ...
                if time_diff.total_seconds() > 0:
                    ...
                    print(
                        f"[MARKET CLOSE] {next_close_et.strftime('%Y-%m-%d %H:%M:%S %Z')}"
                    )
                    print(
                        f"[TRADING TIME] {hours:02d}:{minutes:02d}:{seconds:02d} remaining"
                    )
