...
class OrderManager:
    ...
    def is_trading_allowed(self, symbol):
        ...
        if time_diff < cooldown_minutes:
            self.logger.info(
                f"[{symbol}] Trade cooldown active: {time_diff:.1f}/{cooldown_minutes} minutes"
            )
            return False
    ...
    def get_stock_thresholds(self, symbol):
        ...
        if STOCK_SPECIFIC_AVAILABLE and config["USE_STOCK_SPECIFIC_THRESHOLDS"]:
            ...
            self.logger.info(
                f"[{symbol}] Using stock-specific thresholds: "
    ...
    def place_buy_order(self, symbol, signal_data):
        ...
        try:
            ...
            if existing_position:
                self.logger.info(
                    f"[SAFETY BLOCK] {symbol} - Already have long position: {existing_position['qty']} shares"
                )
                return None
            ...
            if len(long_positions) >= max_positions:
                self.logger.info(
                    f"[POSITION LIMIT] Cannot open new position - already have {len(long_positions)}/{max_positions} positions"
                )
                return None
            ...
            if not current_price or not account_info:
                self.logger.error(
                    f"[ERROR] Could not get price or account info for {symbol}"
                )
                return None
            ...
            self.logger.info(
                f"[ORDER] Stop Loss: ${stop_loss_price:.2f} ({thresholds['stop_loss_pct']*100:.2f}%)"
            )
            self.logger.info(
                f"[ORDER] Take Profit: ${take_profit_price:.2f} ({thresholds['take_profit_pct']*100:.2f}%)"
            )
            self.logger.info(
                f"[ORDER] Strategy: {signal_data.get('strategy', 'Unknown')}, "
            ...
            try:
                ...
                self.logger.info(
                    f"[SUCCESS] Stop loss order placed - Order ID: {stop_order.id}"
                )
                ...
                if "wash trade" in error_msg or "complex orders" in error_msg:
                    self.logger.warning(
                        f"[WASH_TRADE] {symbol}: Stop loss blocked by wash trade rules"
                    )
                    self.logger.info(
                        f"[WASH_TRADE] {symbol}: Trailing stop manager will handle protection"
                    )
                else:
            ...
            self.logger.info(
                f"🛡️  Initial Stop: ${stop_loss_price:.2f} (-{thresholds['stop_loss_pct']:.1%})"
            )
            self.logger.info(
                f"🎯 Take Profit: ${take_profit_price:.2f} (+{thresholds['take_profit_pct']:.1%})"
            )

            if config["TRAILING_STOP_ENABLED"]:
                activation_price = current_price * (
                    1 + thresholds["trailing_activation_pct"]
                )
                self.logger.info(
                    f"🚀 Trailing Activation: ${activation_price:.2f} (+{thresholds['trailing_activation_pct']:.1%})"
                )
                self.logger.info(
                    f"📏 Trailing Distance: {thresholds['trailing_distance_pct']:.1%}"
                )
            ...
            if "wash trade" in error_msg or "complex orders" in error_msg:
                self.logger.warning(
                    f"[WASH_TRADE] {symbol}: Alpaca blocked BUY order due to wash trade rules"
                )
                self.logger.warning(
                    f"[WASH_TRADE] {symbol}: This is a paper trading restriction, real trading would work"
                )
                self.logger.info(
                    f"[WASH_TRADE] {symbol}: Skipping order due to wash trade prevention"
                )
                return None
            else:
                self.logger.error(
                    f"[ERROR] Failed to place buy order for {symbol}: {e}"
                )
                return None
    ...
    def place_sell_order(self, symbol, qty=None):
        ...
        try:
            ...
            if "wash trade" in error_msg or "complex orders" in error_msg:
                self.logger.warning(
                    f"[WASH_TRADE] {symbol}: Alpaca blocked order due to wash trade rules"
                )
                self.logger.warning(
                    f"[WASH_TRADE] {symbol}: This is a paper trading restriction, real trading would work"
                )
                self.logger.info(
                    f"[WASH_TRADE] {symbol}: Marking position as 'virtually closed' for tracking"
                )
                ...
                self.logger.error(
                    f"[ERROR] Failed to place sell order for {symbol}: {e}"
                )
                return None
    ...
    def _simulate_position_close(self, symbol, qty):
        ...
        try:
            self.logger.info(
                f"[SIMULATION] {symbol}: Simulating closure of {qty} shares"
            )
            ...
            if hasattr(self, "trailing_stop_manager"):
                ...
                self.logger.info(
                    f"[SIMULATION] {symbol}: Removed from trailing stop tracking"
                )
            ...
            self.logger.info(
                f"[SIMULATION] {symbol}: Position closure simulated successfully"
            )

        except Exception as e:
            self.logger.error(
                f"[SIMULATION] Error simulating position close for {symbol}: {e}"
            )
    ...
    def update_trailing_stops(self, symbol: str, current_price: float):
        ...
        try:
            ...
            update_info = self.trailing_stop_manager.update_position_price(
            ...
            if update_info and update_info["action"] == "update_stop":
                ...
                if symbol in self.trailing_stop_manager.stop_orders:
                    try:
                        ...
                        self.logger.info(
                            f"[{symbol}] Cancelled old stop order: {old_order_id}"
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"[{symbol}] Failed to cancel old stop order: {e}"
                        )

                # Place new trailing stop order
                try:
                    position_status = self.trailing_stop_manager.get_position_status(
                        symbol
                    )
                    if position_status:
                        ...
                        if not validate_price_precision(
                            stop_price, f"{symbol} updated_stop"
                        ):
                            self.logger.warning(
                                f"[{symbol}] Updated stop price precision issue: {stop_price}"
                            )
                        ...
                        self.trailing_stop_manager.stop_orders[symbol] = (
                            new_stop_order.id
                        )
                        self.logger.info(
                            f"[{symbol}] New trailing stop order placed: {new_stop_order.id} at ${stop_price:.2f}"
                        )

                except Exception as e:
                    self.logger.error(
                        f"[{symbol}] Failed to place new trailing stop order: {e}"
                    )
            ...
            for symbol in list(self.trailing_stop_manager.active_positions.keys()):
                ...
                try:
                    ...
                    if current_price:
                        ...
                        if self.trailing_stop_manager.check_stop_triggered(
                            symbol, current_price
                        ):
                            ...
                            position_status = (
                                self.trailing_stop_manager.get_position_status(symbol)
                            )
                            if position_status:
                                self.logger.warning(
                                    f"[{symbol}] 🛑 EXECUTING TRAILING STOP SELL"
                                )
                                ...
                                if sell_result:
                                    self.trailing_stop_manager.remove_position(
                                        symbol, "Trailing stop triggered"
                                    )

                except Exception as e:
                    self.logger.error(
                        f"[{symbol}] Failed to check trailing stop trigger: {e}"
                    )
    ...
    def place_short_order(self, symbol, signal_data):
        ...
        try:
            ...
            if signal_confidence < min_confidence:
                self.logger.info(
                    f"[SKIP] Short confidence too low: {signal_confidence:.1f}% < {min_confidence}%"
                )
                return None
            ...
            if not current_price or not account_info:
                self.logger.error(
                    f"[ERROR] Could not get price/account data for {symbol}"
                )
                return None
            ...
            shares = int(
                min(max_short_position, account_info["equity"] * 0.01) / current_price
            )  # 1% of equity max

            if shares < 1:
                self.logger.warning(
                    f"[WARNING] Position size too small for {symbol}: {shares} shares"
                )
                return None
            ...
            stop_loss_price = round_to_cent(
                current_price * (1 + thresholds["stop_loss_pct"])
            )
            take_profit_price = round_to_cent(
                current_price * (1 - thresholds["take_profit_pct"])
            )
            ...
            if (
                required_cash > account_info["buying_power"] * 0.5
            ):  # Conservative margin usage
                self.logger.warning(
                    f"[WARNING] Insufficient buying power for short {symbol}"
                )
                return None
            ...
            try:
                ...
                self.logger.info(
                    f"[SUCCESS] Short stop loss order placed - Order ID: {stop_order.id}"
                )

            except Exception as stop_error:
                self.logger.error(
                    f"[ERROR] Failed to place stop loss for short {symbol}: {stop_error}"
                )
    ...
    def place_cover_order(self, symbol, qty=None):
        ...
        try:
            ...
            if qty is None:
                ...
                if not position:
                    self.logger.warning(
                        f"[WARNING] No short position found for {symbol}"
                    )
                    return None
        ...
        try:
            ...
            for order in orders:
                if order.order_type.name == "STOP" and order.symbol == symbol:
                    self.logger.info(
                        f"[CANCEL] Cancelling existing stop order {order.id} for {symbol}"
                    )
            ...
            if cancelled_count > 0:
                self.logger.info(
                    f"[SUCCESS] Cancelled {cancelled_count} stop orders for {symbol}"
                )
            else:
