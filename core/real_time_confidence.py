...
class RealTimeConfidenceCalculator:
    def __init__(self):
        ...
        self.min_confidence_threshold = (
            70.0  # Match config.py setting (lowered from 75%)
        )
    ...
    def get_live_market_data(
        self, symbol: str, period: str = "5d", interval: str = "15m"
    ):
    ...
    def calculate_technical_indicators(self, data: pd.DataFrame):
        ...
        # Price momentum
        momentum_1h = (
            (data["Close"] - data["Close"].shift(4)) / data["Close"].shift(4) * 100
        )  # 4 periods = 1 hour
        momentum_30m = (
            (data["Close"] - data["Close"].shift(2)) / data["Close"].shift(2) * 100
        )  # 2 periods = 30 min
    ...
    def score_volatility_match(
        self, indicators: dict, expected_volatility: float
    ) -> float:
    ...
    def calculate_real_time_confidence(
        self, symbol: str, expected_volatility: float = 1.0
    ) -> dict:
        """
        ...
        scores = {
            ...
            "volatility_match": self.score_volatility_match(
                indicators, expected_volatility
            ),
        }
        ...
        result = {
            ...
            "technical_summary": {
                "macd_bullish": indicators["macd_line"].iloc[-1]
                > indicators["macd_signal"].iloc[-1],
                "above_ema9": indicators["current_price"]
                > indicators["ema_9"].iloc[-1],
        ...
        print(
            f"[OK] {symbol}: {final_confidence:.1f}% confidence ({'TRADEABLE' if tradeable else 'SKIP'})"
        )
