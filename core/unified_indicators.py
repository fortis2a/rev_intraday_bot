...
class UnifiedIndicatorService:
    ...
    def calculate_unified_indicators(self, df: pd.DataFrame, symbol: str) -> Dict:
        ...
        vwap_std = ((typical_price - indicators["vwap"]) ** 2 * volume).rolling(
            window=20
        ...
        obv = (
            volume.where(close > close.shift(1), -volume)
            .where(close != close.shift(1), 0)
            .cumsum()
        )
        indicators["obv"] = obv
        ...
        result = {
            ...
            "current_values": {
                ...
                "rsi": (
                    indicators["rsi"].iloc[-1] if len(indicators["rsi"]) > 0 else None
                ),
                "macd_bullish": (
                ...
                "above_ema9": (
                    close.iloc[-1] > indicators["ema_9"].iloc[-1]
                    if len(indicators["ema_9"]) > 0
                    else None
                ),
                "above_vwap": (
                    close.iloc[-1] > indicators["vwap"].iloc[-1]
                    if len(indicators["vwap"]) > 0
                    else None
                ),
                "bb_position": (
                    (
                        (close.iloc[-1] - indicators["bb_lower"].iloc[-1])
                        / (
                            indicators["bb_upper"].iloc[-1]
                            - indicators["bb_lower"].iloc[-1]
                        )
                    )
                    if len(indicators["bb_lower"]) > 0
                    else None
                ),
            },
        }
    ...
    def get_indicators_for_strategy(
        self, df: pd.DataFrame, symbol: str, strategy_type: str
    ) -> Dict:
        """
    ...
    def clear_cache(self, symbol: Optional[str] = None):
        ...
        if symbol:
            keys_to_remove = [
                k for k in self._indicator_cache.keys() if k.startswith(symbol)
            ]
