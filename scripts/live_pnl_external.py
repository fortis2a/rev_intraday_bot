...
class LivePnLMonitor:
    ...
    def __init__(self):
        ...
        print(f"Features: All Watchlist Stocks | Individual P&L | Summary P&L | Alpaca-Synced")
    ...
    def initialize_alpaca_daily_tracking(self):
        ...
        try:
            ...
            if account_info:
                ...
                self.logger.info(f"📊 Alpaca last equity (start of day): ${self.start_of_day_equity:,.2f}")
                self.logger.info(f"📊 Current equity: ${current_equity:,.2f}")
                
                # Also get portfolio history to double-check
                try:
                    portfolio_history = self.data_manager.api.get_portfolio_history(period='1D', timeframe='1Min')
                    if hasattr(portfolio_history, 'equity') and portfolio_history.equity:
                        historical_start = portfolio_history.equity[0]
                        self.logger.info(f"📊 Portfolio history start: ${historical_start:,.2f}")
                        ...
                        if abs(historical_start - self.start_of_day_equity) > 0.01:
                            ...
                            self.logger.info(f"📊 Using portfolio history start of day: ${self.start_of_day_equity:,.2f}")
                except Exception as portfolio_error:
                    self.logger.warning(f"Portfolio history not available: {portfolio_error}")
                    
            else:
                self.start_of_day_equity = 100000  # Default fallback
                self.logger.warning("Account info unavailable, using fallback start of day equity")
    ...
    def get_alpaca_daily_pnl(self):
        ...
        try:
            ...
            return {
                ...
                'daily_return_pct': (alpaca_daily_pnl / last_equity * 100) if last_equity > 0 else 0
            }
        except Exception as e:
        ...
        try:
            ...
            for order in orders:
                if hasattr(order, 'filled_avg_price') and order.filled_avg_price:
                    ...
                    if side == 'buy':
                        symbol_positions[symbol]['buys'].append({'qty': qty, 'price': price})
                    else:
                        symbol_positions[symbol]['sells'].append({'qty': qty, 'price': price})
    ...
    def display_account_summary(self, account_info):
        ...
        if 'day_trading_buying_power' in account_info:
            print(f"Day Trading Power:   ${account_info['day_trading_buying_power']:,.2f}")
    ...
    def display_positions(self, positions):
        ...
        print("-" * 95)
        print(f"{'Symbol':<8} {'Qty':<8} {'Side':<6} {'Entry $':<10} {'Current $':<10} {'P&L $':<12} {'P&L %':<8} {'Mkt Val $':<12}")
        print("-" * 95)
        ...
        for pos in positions:
            ...
            pnl_pct = (unrealized_pl / (abs(qty) * avg_price)) * 100 if qty != 0 and avg_price != 0 else 0
            
            pnl_str = self.format_currency(unrealized_pl)
            pnl_pct_str = f"{pnl_pct:+.2f}%"
            
            print(f"{symbol:<8} {qty:<8} {side:<6} ${avg_price:<9.2f} ${current_price:<9.2f} "
                  f"{pnl_str:<12} {pnl_pct_str:<8} ${market_value:<11.2f}")
        
        print("-" * 95)
        print(f"{'TOTALS:':<50} {self.format_currency(total_unrealized):<12} {'':>8} ${total_market_value:<11.2f}")
    ...
    def display_watchlist_overview(self, positions):
        ...
        print("-" * 105)
        print(f"{'Symbol':<8} {'Status':<12} {'Qty':<8} {'Entry $':<10} {'Current $':<10} {'P&L $':<12} {'P&L %':<8} {'Mkt Val $':<12}")
        print("-" * 105)
        ...
        for symbol in self.watchlist:
            try:
                ...
                if symbol in position_lookup:
                    ...
                    print(f"{symbol:<8} {status:<12} {qty:<8} ${avg_price:<9.2f} ${current_price:<9.2f} "
                          f"{pnl_str:<12} {pnl_pct_str:<8} ${market_value:<11.2f}")
                else:
                    # Stock is on watchlist but no position
                    if current_price:
                        print(f"{symbol:<8} {'NO POSITION':<12} {'-':<8} {'-':<10} ${current_price:<9.2f} "
                              f"{'$0.00':<12} {'+0.00%':<8} {'$0.00':<12}")
                    else:
                        print(f"{symbol:<8} {'PRICE ERROR':<12} {'-':<8} {'-':<10} {'-':<10} "
                              f"{'$0.00':<12} {'+0.00%':<8} {'$0.00':<12}")
                        
            except Exception as e:
                self.logger.error(f"Error getting data for {symbol}: {e}")
                print(f"{symbol:<8} {'ERROR':<12} {'-':<8} {'-':<10} {'-':<10} "
                      f"{'$0.00':<12} {'+0.00%':<8} {'$0.00':<12}")
        
        print("-" * 105)
        print(f"{'POSITION TOTALS:':<60} {self.format_currency(total_unrealized):<12} {'':>8} ${total_market_value:<11.2f}")
    ...
    def display_watchlist_summary(self):
        ...
        try:
            ...
            positions_count = len([s for s in self.watchlist if any(pos['symbol'] == s for pos in positions)])
            print(f"Active Positions: {positions_count}/{len(self.watchlist)} watchlist stocks")
    ...
    def run_monitor(self):
        ...
        while self.running:
            try:
                ...
                total_unrealized = sum(p['unrealized_pl'] for p in positions) if positions else 0
                alpaca_pnl_data = self.get_alpaca_daily_pnl()
                
                self.logger.info(f"💰 Equity: ${alpaca_pnl_data['current_equity']:,.2f} | "
