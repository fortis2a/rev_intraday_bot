...
class IntradayStockFinder:
    ...
    def get_candidate_stocks(self):
        ...
        candidates = [
            # Tech/Growth
            'AMD', 'NVDA', 'TSLA', 'PLTR', 'SOFI', 'NIO', 'BABA', 'F', 'BAC',
            'INTC', 'CSCO', 'ORCL', 'CRM', 'ADBE', 'PYPL', 'SQ', 'ROKU', 'ZM',
            'UBER', 'LYFT', 'SNAP', 'TWTR', 'PINS', 'SPOT', 'COIN', 'HOOD',
            
            # Financial
            'JPM', 'WFC', 'C', 'GS', 'MS', 'AXP', 'V', 'MA', 'COF', 'SCHW',
            
            # Healthcare/Biotech
            'JNJ', 'PFE', 'ABBV', 'MRK', 'UNH', 'CVS', 'GILD', 'AMGN', 'BIIB',
            'MRNA', 'BNTX', 'NVAX', 'PFE', 'ABT', 'TMO', 'DHR',
            
            # Consumer
            'DIS', 'NFLX', 'SBUX', 'MCD', 'KO', 'PEP', 'WMT', 'TGT', 'HD', 'LOW',
            'NKE', 'LULU', 'COST', 'AMZN', 'ETSY', 'SHOP',
            ...
            'CAT', 'DE', 'BA', 'LMT', 'RTX', 'GE', 'MMM', 'UPS', 'FDX', 'DAL', 'UAL',
            
            # ETFs with good intraday movement
            'SPY', 'QQQ', 'IWM', 'XLF', 'XLE', 'XLK', 'XLV', 'XLI', 'XLU',
            'SOXL', 'TQQQ', 'SPXL', 'FAS', 'TNA', 'LABU'
        ]
    ...
    def filter_by_price_volume(self, stocks):
        ...
        print(f"📊 Filtering {len(stocks)} stocks by price (<${self.max_price}) and volume (>{self.min_volume:,})...")
        ...
        for i, symbol in enumerate(stocks):
            try:
                ...
                # Filter criteria
                if current_price > 0 and current_price <= self.max_price and avg_volume >= self.min_volume:
    ...
    def analyze_intraday_suitability(self, stock_info):
        ...
        try:
            ...
            clean_returns = returns_clean[abs(returns_clean - mean_return) <= 3 * std_return]
            
            # Calculate key metrics
            avg_volatility = data['volatility'].mean() * 100  # Convert to percentage
            avg_range = data['range_pct'].mean()
            avg_volume = data['Volume'].mean()
            volume_consistency = data['Volume'].std() / avg_volume if avg_volume > 0 else float('inf')
            ...
            daily_data['daily_range_pct'] = ((daily_data['High'] - daily_data['Low']) / daily_data['Close']) * 100
            ...
            volume_score = min((avg_volume / 2000000) * 30, 30)  # Max 30 points for volume
            ...
            range_score = min((avg_daily_range / 3) * 25, 25)  # Max 25 points for 3%+ daily range
            
            total_score = volume_score + volatility_score + consistency_score + range_score
            ...
            print(f"   ✅ {symbol}: Score {total_score:.1f}/100 ({analysis['trading_suitability']})")
            return analysis
...
def main():
    ...
    if results:
        ...
        for i, stock in enumerate(results[:10]):
            print(f"{i+1}. {stock['symbol']}: {stock['total_score']:.1f}/100 ({stock['trading_suitability']})")
