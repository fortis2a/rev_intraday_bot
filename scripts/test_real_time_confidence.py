...
def compare_confidence_methods():
    ...
    print(f"\n📊 COMPARISON TABLE:")
    print("=" * 70)
    print(f"{'Stock':<6} {'Historical':<12} {'Real-Time':<12} {'Difference':<12} {'Status'}")
    print("-" * 70)
    ...
    for symbol in budget_watchlist:
        ...
        print(f"{symbol:<6} {hist:<12.1f} {real_time:<12.1f} {diff:+.1f}%{'':<7} {status}")
    ...
    historical_filtered = get_filtered_watchlist(budget_watchlist, 75.0, use_real_time=False)
    
    print("\n🔄 Real-time filtering:")
    real_time_filtered = get_filtered_watchlist(budget_watchlist, 75.0, use_real_time=True)
    
    print(f"\n📋 FILTERING RESULTS SUMMARY:")
    print(f"Historical method: {len(historical_filtered)}/{len(budget_watchlist)} stocks passed")
    print(f"Real-time method: {len(real_time_filtered)}/{len(budget_watchlist)} stocks passed")
