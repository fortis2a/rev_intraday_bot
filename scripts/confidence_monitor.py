...
class ConfidenceMonitor:
    ...
    def get_confidence_data(self, symbol):
        ...
        try:
            ...
            return {
                ...
                'timestamp': primary_decision.get('timestamp', datetime.now().strftime('%H:%M:%S'))
            }
    ...
    def display_confidence_dashboard(self):
        ...
        print("=" * 120)
        print(f"⏰ Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 🎯 Threshold: {self.confidence_threshold}% | 🔄 Refresh: {self.refresh_interval}s")
        print()
        ...
        # Display header
        print(f"{'Symbol':<6} {'Confidence':<11} {'Direction':<9} {'Alt':<7} {'MACD':<5} {'EMA9':<5} {'VWAP':<5} {'RSI':<6} {'Vol':<6} {'Stop%':<6} {'Profit%':<7} {'Updated':<8}")
        print("-" * 120)
        ...
        for data in confidence_data:
            ...
            rsi_str = f"{data['rsi']}" if isinstance(data['rsi'], (int, float)) else "N/A"
            vol_str = f"{data['volume']:.1f}x" if isinstance(data['volume'], (int, float)) else "N/A"
            stop_str = f"{data['stop_pct']:.2f}%"
            profit_str = f"{data['profit_pct']:.2f}%"
            
            print(f"{data['symbol']:<6} {conf_str:<11} {data['status']:<9} {alt_str:<7} {data['macd']:<5} {data['ema9']:<5} {data['vwap']:<5} {rsi_str:<6} {vol_str:<6} {stop_str:<6} {profit_str:<7} {data['timestamp']:<8}")
        
        print("-" * 120)
        print(f"📊 SUMMARY: {tradeable_count}/{len(self.watchlist)} stocks above {self.confidence_threshold}% threshold")
        
        # Trading recommendations
        print(f"\n🎯 TRADING RECOMMENDATIONS:")
        tradeable_stocks = [data for data in confidence_data if data['confidence'] >= self.confidence_threshold]
        
        if tradeable_stocks:
            print(f"✅ READY TO TRADE ({len(tradeable_stocks)} stocks):")
            for stock in tradeable_stocks:
                print(f"   • {stock['symbol']}: {stock['confidence']:.1f}% confidence - {stock['direction']} (Alt: {stock['alt_direction']} {stock['alt_confidence']:.0f}%)")
        else:
            print(f"⚠️  NO STOCKS currently meet the {self.confidence_threshold}% threshold")
        
        # Warning for low confidence stocks
        low_confidence = [data for data in confidence_data if data['confidence'] < self.confidence_threshold]
        if low_confidence:
            print(f"\n🚫 AVOID TRADING ({len(low_confidence)} stocks below threshold):")
            for stock in low_confidence[:3]:  # Show top 3 closest to threshold
                print(f"   • {stock['symbol']}: {stock['confidence']:.1f}% confidence - {stock['status']}")
        
        print(f"\n💡 LEGEND: 🟢=Trade Ready | 🟡=Review Required | 🔴=Skip | ✅=Bullish | ❌=Bearish")
        print(f"⏰ Monitoring every {self.refresh_interval} seconds - Press Ctrl+C to stop")
    
    def run_monitor(self):
        """Run the continuous monitoring loop"""
        print("🎯 Starting Real-Time Confidence Monitor...")
        print(f"📊 Monitoring {len(self.watchlist)} stocks: {', '.join(self.watchlist)}")
        ...
        try:
            ...
            print(f"📊 Monitor can be restarted anytime using: python scripts/confidence_monitor.py")
        except Exception as e:
