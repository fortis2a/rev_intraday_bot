...
def create_timeline_chart(df):
    ...
    hourly_data = df_copy.groupby(['hour', 'side']).agg({
        'qty': 'sum',
        'value': 'sum'
    }).reset_index()
...
def create_cumulative_pnl_chart(stock_pnl):
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='tozeroy',
        mode='none',
        fillcolor='rgba(0,100,80,0.2)' if cumulative[-1] > 0 else 'rgba(255,0,0,0.2)',
        showlegend=False,
        hoverinfo='skip'
    ))
...
def main():
    ...
    st.markdown('<h1 class="main-header">🚀 Interactive Scalping Bot Dashboard</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; font-size: 0.8rem; margin: 0; padding: 0; color: #666; line-height: 1;">Real-time trading analytics with live Alpaca data</p>', unsafe_allow_html=True)
    ...
    st.sidebar.info(f"📊 Selected: {days_selected} day{'s' if days_selected != 1 else ''}")
    ...
    chart_height = st.sidebar.slider("📏 Chart Height (px):", 140, 220, 160, step=20, 
    ...
    if not df_filtered.empty:
        ...
        st.markdown(f"<small>📊 **Data:** {len(df_filtered)} trades | {(end_date - start_date).days + 1} days | {len(df_filtered['symbol'].unique())} symbols</small>", unsafe_allow_html=True)
    ...
    if account_data:
        ...
        with col3:
            st.metric(
                label="📊 Day Trades",
                value=account_data['day_trade_count']
            )
    ...
    st.markdown('<h4 style="margin: 0.2rem 0; padding: 0;">📊 Interactive Analytics</h4>', unsafe_allow_html=True)
    ...
    with tab1:
        st.info("🎯 **All analytics are displayed above** - Use sidebar filters to customize your view")
    ...
    with tab2:
        ...
        if selected_stocks:
            ...
            # Format for display
            display_data = table_data[['symbol', 'side', 'qty', 'price', 'value', 'filled_at']].copy()
            display_data['filled_at'] = display_data['filled_at'].dt.strftime('%m/%d %H:%M:%S')
            ...
            st.dataframe(styled_df, use_container_width=True, height=400)  # Full height for data page
