...
class ConfidenceCalculator:
    ...
    def __init__(self):
        ...
        self.symbols = INTRADAY_WATCHLIST if INTRADAY_WATCHLIST else ["SOXL", "SOFI", "TQQQ", "INTC", "NIO"]
    ...
    async def calculate_confidence_for_symbol(self, symbol: str, market_data: Dict = None) -> Dict:
        ...
        try:
            ...
            indicators = await self.indicator_service.calculate_all_indicators(symbol, market_data)
            ...
            for strategy_name, strategy in self.strategies.items():
                try:
                    ...
                    result = await self.get_strategy_confidence(strategy, symbol, indicators, market_data)
                    strategy_confidences[strategy_name] = result['confidence']
                    strategy_signals[strategy_name] = result['signal']
                except Exception as e:
                    self.logger.error(f"Error calculating {strategy_name} confidence for {symbol}: {e}")
            ...
            overall_signal = self.determine_overall_signal(strategy_signals, overall_confidence)
    ...
    async def get_strategy_confidence(self, strategy, symbol: str, indicators: Dict, market_data: Dict) -> Dict:
        ...
        try:
            ...
            if hasattr(strategy, 'calculate_confidence'):
                result = await strategy.calculate_confidence(symbol, indicators, market_data)
                return result
            else:
                # Fallback simulation based on strategy type
                return self.simulate_strategy_confidence(strategy.__class__.__name__, indicators)
                
        except Exception as e:
            self.logger.error(f"Error getting strategy confidence: {e}")
            return {'confidence': 50.0, 'signal': 'HOLD'}
            
    def simulate_strategy_confidence(self, strategy_name: str, indicators: Dict) -> Dict:
    ...
    def determine_overall_signal(self, strategy_signals: Dict, overall_confidence: float) -> str:
    ...
    async def get_market_data(self, symbol: str) -> Dict:
        ...
        base_prices = {
            "SOXL": 29.27, "SOFI": 23.81, "TQQQ": 94.86, 
            "INTC": 22.22, "NIO": 4.62
        }
    ...
    async def calculate_all_symbols(self) -> Dict[str, Dict]:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                self.logger.error(f"Error calculating confidence for {symbol}: {result}")
    ...
    def get_symbol_confidence(self, symbol: str) -> Dict:
        ...
        base_prices = {
            ...
            "AAPL": 185.0, "MSFT": 365.0, "GOOGL": 145.0, "TSLA": 255.0, "NVDA": 465.0
        }
