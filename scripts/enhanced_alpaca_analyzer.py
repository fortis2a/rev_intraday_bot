...
class EnhancedAlpacaAnalyzer:
    ...
    def analyze_trading_session(self, orders):
        ...
        try:
            ...
            for symbol, trades in symbol_groups.items():
                ...
                if buys:
                    symbol_stats["avg_buy_price"] = (
                        symbol_stats["buy_value"] / symbol_stats["buy_volume"]
                    )
                if sells:
                    symbol_stats["avg_sell_price"] = (
                        symbol_stats["sell_value"] / symbol_stats["sell_volume"]
                    )
            ...
            session_stats = {
                ...
                "profitable_symbols": len(
                    [s for s in analysis_results.values() if s["realized_pnl"] > 0]
                ),
                "losing_symbols": len(
                    [s for s in analysis_results.values() if s["realized_pnl"] < 0]
                ),
            ...
            return {"session_stats": session_stats, "symbol_analysis": analysis_results}
    ...
    def display_enhanced_analysis(self):
        ...
        for symbol, stats in symbol_analysis.items():
            print(f"\n🔍 {symbol}")
            print(
                f"   Orders: {stats['total_buys']} buys, {stats['total_sells']} sells"
            )
            print(
                f"   Volume: {stats['buy_volume']:.0f} bought, {stats['sell_volume']:.0f} sold"
            )
            print(
                f"   Avg Prices: Buy ${stats['avg_buy_price']:.4f}, Sell ${stats['avg_sell_price']:.4f}"
            )
            print(
                f"   Total Value: ${stats['buy_value']:,.2f} bought, ${stats['sell_value']:,.2f} sold"
            )
            ...
            if stats["trades"]:
                ...
                for i, trade in enumerate(stats["trades"][:5]):  # Show first 5 trades
                    ...
                    print(
                        f"     Trade {i+1}: {trade['qty']:.0f} shares @ "
        ...
        if alpaca_pnl:
            ...
            if difference < 0.01:
                ...
                print(
                    f"✅ Close match (diff: ${difference:.2f}) - likely rounding/timing differences"
                )
            else:
        ...
        if session_stats["total_completed_trades"] > 0:
            avg_trade_pnl = (
                session_stats["total_realized_pnl"]
                / session_stats["total_completed_trades"]
            )
            win_rate = (
                len([s for s in symbol_analysis.values() if s["realized_pnl"] > 0])
                / len(symbol_analysis)
                * 100
            )
            ...
            print(
                f"Total Return on Volume:    {(session_stats['total_realized_pnl'] / session_stats['total_volume_traded'] * 100):+.3f}%"
            )

            # Best and worst performing symbols
            if symbol_analysis:
                best_symbol = max(
                    symbol_analysis.values(), key=lambda x: x["realized_pnl"]
                )
                worst_symbol = min(
                    symbol_analysis.values(), key=lambda x: x["realized_pnl"]
                )
                ...
                print(
                    f"Best Performer:            {best_symbol['symbol']} (${best_symbol['realized_pnl']:+.2f})"
                )
                print(
                    f"Worst Performer:           {worst_symbol['symbol']} (${worst_symbol['realized_pnl']:+.2f})"
                )
