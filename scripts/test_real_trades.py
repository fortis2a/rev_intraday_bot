...
try:
    from scripts.alpaca_connector import alpaca_connector, get_real_trade_history, get_real_strategy_performance
...
async def test_alpaca_connection():
    ...
    try:
        ...
        if is_connected:
            ...
            if account:
                print(f"✅ Account Data: Equity=${account.equity:,.2f}, Cash=${account.cash:,.2f}")
            else:
                print("⚠️ Could not fetch account data")
                
            return True
        else:
            print(f"❌ Alpaca API connection failed: {alpaca_connector.connection_error}")
            return False
    ...
    try:
        ...
        if trades:
            ...
            for i, trade in enumerate(trades[:5]):  # Show first 5
                print(f"  {i+1}. {trade['symbol']} {trade['action']} {trade['quantity']}@${trade['price']:.2f} "
            ...
            print("ℹ️ No recent trades found - this is normal for a new paper trading account")
...
async def main():
    ...
    if connected:
        ...
        print("💡 Command Center will now pull actual trades from your Alpaca paper account")
    else:
