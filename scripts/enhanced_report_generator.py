...
class DatabaseReportGenerator:
    ...
    def generate_market_close_report(self, target_date: date = None) -> str:
        ...
        report_file = (
            self.report_dir
            / f"market_close_report_{target_date.strftime('%Y%m%d')}.html"
        )
    ...
    def _generate_html_report(
        self, target_date: date, today: dict, yesterday: dict, activities: pd.DataFrame
    ) -> str:
        ...
        if today:
            ...
            today_trading_pnl = today.get(
                "trading_pnl", today_alpaca_pnl
            )  # Use Alpaca as fallback
        ...
        # Yesterday's metrics
        yesterday_cash_flow_pnl = (
            yesterday.get("cash_flow_pnl", yesterday.get("net_pnl", 0))
            if yesterday
            else 0.0
        )
        yesterday_alpaca_pnl = (
            yesterday.get("alpaca_pnl", yesterday_cash_flow_pnl) if yesterday else 0.0
        )
        yesterday_trading_pnl = (
            yesterday.get("trading_pnl", yesterday_alpaca_pnl) if yesterday else 0.0
        )
        ...
        if not activities.empty:
            symbol_stats = (
                activities.groupby("symbol")
                .agg({"id": "count", "value": "sum"})
        ...
        if not activities.empty:
            ...
            for _, trade in recent.iterrows():
                time_str = pd.to_datetime(trade["transaction_time"]).strftime(
                    "%H:%M:%S"
                )
                side_color = (
                    "green" if trade["side"] in ["sell", "sell_short"] else "red"
                )
    ...
    def generate_weekly_summary(self) -> str:
        ...
        report_file = (
            self.report_dir / f"weekly_summary_{end_date.strftime('%Y%m%d')}.html"
        )
        ...
        for _, row in summaries.iterrows():
            pnl_class = (
                "positive"
                if row["net_pnl"] > 0
                else "negative" if row["net_pnl"] < 0 else ""
            )
            html += f"""
                    <tr>
...
if __name__ == "__main__":
    print(
        f"📊 Generating Market Close Report - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
