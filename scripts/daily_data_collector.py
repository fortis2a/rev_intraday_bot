...
def collect_today_data():
    ...
    try:
        ...
        if today.weekday() >= 5:  # Saturday=5, Sunday=6
            logger.info(f"Market closed today ({today.strftime('%A')}), skipping collection")
            return True
        ...
        if success:
            if count > 0:
                logger.info(f"✅ Successfully collected {count} trading activities for {today}")
                
                # Get summary for verification
                summary = db.get_daily_summary(today)
                if summary:
                    alpaca_pnl = summary.get('alpaca_pnl', summary.get('net_pnl', 0))
                    logger.info(f"📊 Daily Summary: {summary['total_trades']} trades, "
