...
class AlpacaPnLCalculator:
    ...
    def get_account_activities(self, activity_type=None):
        ...
        try:
            ...
            self.logger.info(f"Found {len(activities)} activities today (type: {activity_type or 'ALL'})")
            return activities
    ...
    def generate_comprehensive_pnl_report(self):
        ...
        if orders:
            realized_pnl, order_pnl_details = self.calculate_realized_pnl_from_orders(orders)
        ...
        print("-" * 50)
        activity_realized, activity_details = self.calculate_realized_pnl_from_activities()
        ...
        if position_details:
            ...
            for symbol, pos in position_details.items():
                ...
                print(f"  {symbol:>6}: {side} {abs(qty):>3.0f} @ ${entry:>7.2f} | "
