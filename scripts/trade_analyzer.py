...
class TradeDecisionAnalyzer:
    ...
    def _analyze_log_files(self, log_files: List[Path], days: int) -> Dict:
                    ...
                    for line in f:
                        ...
                        if "Signal Generated" in line or "EXECUTE TRADE" in line:
                            ...
                            if "strategy" in line.lower():
                                for strategy in ["mean_reversion", "momentum_scalp", "vwap_bounce"]:
                                    if strategy in line.lower():
                            ...
                            if "reason:" in line.lower():
                                try:
                                    ...
                                    decision_reasons.append(reason[:100])  # Limit length
                                except:
                                    pass
        ...
        return {
            ...
            'execution_rate': (trades_executed / signals_found * 100) if signals_found > 0 else 0,
    ...
    def analyze_indicator_effectiveness(self, analysis_data: Dict) -> Dict:
        ...
        if 'decision_reasons' in analysis_data:
            for i, reason in enumerate(analysis_data['decision_reasons']):
                confidence = analysis_data['confidence_scores'][i] if i < len(analysis_data['confidence_scores']) else 75
                
                # Count indicator mentions
                indicators = ['RSI', 'MACD', 'ADX', 'VWAP', 'Bollinger', 'EMA', 'Williams', 'ROC', 'Stochastic']
                for indicator in indicators:
                    if indicator.lower() in reason.lower():
                        indicator_mentions[indicator] = indicator_mentions.get(indicator, 0) + 1
        ...
        for indicator, confidences in confidence_by_indicator.items():
            indicator_effectiveness[indicator] = {
                ...
                'success_rate': len([c for c in confidences if c > 80]) / len(confidences) * 100
            }
        
        # Sort by effectiveness
        sorted_indicators = sorted(indicator_effectiveness.items(), 
                                 key=lambda x: x[1]['avg_confidence'], reverse=True)
        
        print("Top Performing Indicators:")
        for indicator, stats in sorted_indicators[:5]:
            print(f"  📈 {indicator}: {stats['avg_confidence']:.1f}% avg confidence, "
    ...
    def analyze_strategy_performance(self, analysis_data: Dict) -> Dict:
        ...
        for strategy, count in strategies.items():
            ...
            strategy_performance[strategy] = {
                ...
                'recommendation': self._get_strategy_recommendation(strategy, usage_pct, success_rates.get(strategy, 70))
            }
        
        # Display results
        for strategy, stats in strategy_performance.items():
            print(f"  🎲 {strategy.replace('_', ' ').title()}:")
            print(f"     Signals: {stats['signals_generated']} ({stats['usage_percentage']:.1f}%)")
            print(f"     Success Rate: {stats['estimated_success_rate']}%")
            print(f"     💡 {stats['recommendation']}")
        
        return strategy_performance
    
    def _get_strategy_recommendation(self, strategy: str, usage_pct: float, success_rate: float) -> str:
    ...
    def generate_optimization_recommendations(self, analysis_data: Dict) -> List[str]:
        ...
        if execution_rate < 40:
            recommendations.append("🔧 LOW EXECUTION RATE: Consider lowering confidence thresholds or adjusting filters")
        elif execution_rate > 80:
            recommendations.append("⚠️  HIGH EXECUTION RATE: Consider raising quality filters to reduce false signals")
        
        # Confidence analysis
        avg_confidence = analysis_data.get('avg_confidence', 0)
        if avg_confidence < 70:
            recommendations.append("📊 LOW CONFIDENCE: Review indicator weightings and strategy parameters")
        elif avg_confidence > 90:
            recommendations.append("🎯 HIGH CONFIDENCE: System is working well, consider increasing position sizes")
        ...
        if strategies:
            ...
            if most_used[1] > sum(strategies.values()) * 0.7:
                recommendations.append(f"⚖️  STRATEGY IMBALANCE: {most_used[0]} is overused - diversify signal sources")
        
        # Volume of signals
        total_signals = analysis_data.get('signals_generated', 0)
        if total_signals < 5:
            recommendations.append("📈 LOW SIGNAL VOLUME: Consider expanding watchlist or relaxing entry criteria")
        elif total_signals > 50:
            recommendations.append("🛑 HIGH SIGNAL VOLUME: Consider tightening filters to focus on highest quality setups")
    ...
    def create_decision_audit_report(self, symbol: str = None, days: int = 1) -> str:
        ...
        # Save report
        report_file = self.data_dir / f"decision_audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument('--report', action='store_true', help='Generate detailed audit report')
