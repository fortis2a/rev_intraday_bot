        ...
        try:
            ...
            if side == "long":
                ...
                trailing_order = self.api.submit_order(
                    ...
                    type="trailing_stop",
                    trail_price=round(
                        protection_levels["entry_price"] * 0.005, 2
                    ),  # 0.5% trail
                    time_in_force="gtc",
                )
            ...
            self.logger.info(
                f"[{symbol}] Created {len(orders_created)} protection orders"
            )
    ...
    def protect_all_positions(self):
        ...
        for position in positions:
            if float(position.unrealized_pl) > 0:  # Only protect profitable positions
                ...
                self.logger.info(
                    f"[{position.symbol}] Side: {protection_levels['side'].upper()}"
                )
                self.logger.info(
                    f"[{position.symbol}] Entry: ${protection_levels['entry_price']:.2f}"
                )
                self.logger.info(
                    f"[{position.symbol}] Current: ${protection_levels['current_price']:.2f}"
                )
                self.logger.info(
                    f"[{position.symbol}] Profit: {protection_levels['profit_pct']:.2f}%"
                )
                self.logger.info(
                    f"[{position.symbol}] Unrealized P&L: ${protection_levels['unrealized_pl']:.2f}"
                )
                ...
                self.logger.info(
                    f"[{position.symbol}] ✅ Protection {'ACTIVE' if orders else 'FAILED'}"
                )
        ...
        for summary in protection_summary:
            ...
            self.logger.info(
                f"{summary['symbol']} ({summary['side'].upper()}): "
                f"{summary['profit_pct']:+.2f}% (${summary['unrealized_pl']:+.2f}) - {status}"
            )

        self.logger.info(f"{'='*50}")
        self.logger.info(
            f"Positions Protected: {total_protected}/{len(protection_summary)}"
        )
        self.logger.info(f"Total Unrealized Profit: ${total_profit:+.2f}")
        self.logger.info(
            f"🛡️  PROFIT PROTECTION {'COMPLETE' if total_protected == len(protection_summary) else 'PARTIAL'}"
        )
