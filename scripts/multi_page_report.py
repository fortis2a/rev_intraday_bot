...
class MultiPageTradingReport:
    ...
    def create_overview_page(self, pdf, all_data):
        ...
        fig.suptitle('TRADING PERFORMANCE OVERVIEW - ALL DAYS', fontsize=28, fontweight='bold', y=0.95)
        ...
        if all_stock_pnl:
            ...
            plt.title('OVERALL STOCK PERFORMANCE', fontsize=20, fontweight='bold', pad=20)
            ...
            for bar, pnl in zip(bars, pnls):
                ...
                plt.text(bar.get_x() + bar.get_width()/2., height + (10 if height > 0 else -30),
        ...
        if daily_totals:
            ...
            bars = plt.bar(range(len(dates)), totals, color=colors, edgecolor='black', linewidth=2)
            ...
            for bar, total in zip(bars, totals):
                height = bar.get_height()
                plt.text(bar.get_x() + bar.get_width()/2., height + (10 if height > 0 else -30),
                        f'${total:.0f}', ha='center', va='bottom' if height > 0 else 'top', 
                        fontsize=14, fontweight='bold')
        ...
        winners = len([p for p in all_stock_pnl.values() if p > 0]) if all_stock_pnl else 0
        total_stocks = len(all_stock_pnl) if all_stock_pnl else 0
        win_rate = (winners / total_stocks * 100) if total_stocks > 0 else 0
        
        best_stock = max(all_stock_pnl.items(), key=lambda x: x[1]) if all_stock_pnl else ('N/A', 0)
        worst_stock = min(all_stock_pnl.items(), key=lambda x: x[1]) if all_stock_pnl else ('N/A', 0)
        
        best_day = max(daily_totals.items(), key=lambda x: x[1]) if daily_totals else ('N/A', 0)
        worst_day = min(daily_totals.items(), key=lambda x: x[1]) if daily_totals else ('N/A', 0)
...
WORST DAY: {worst_day[0]} (${worst_day[1]:+.2f})
        ...
        if daily_totals:
            ...
            for date_str, total in sorted(daily_totals.items()):
                ...
                stocks_traded = len([s for s, p in all_stock_pnl.items() if date_str in str(all_data)])
                status = 'PROFIT' if total > 0 else 'LOSS' if total < 0 else 'BREAK-EVEN'
            ...
            for i in range(1, len(table_data) + 1):
                ...
                color = 'lightgreen' if total > 0 else 'lightcoral' if total < 0 else 'lightyellow'
    ...
    def create_daily_page(self, pdf, date_str, orders):
        ...
        if date_str == '2025-08-12':
            fig.suptitle(f'TRADING ANALYSIS - {day_name}\\nWARNING: NO PROPER RISK MANAGEMENT', 
                        fontsize=24, fontweight='bold', y=0.95, color='red')
        else:
            fig.suptitle(f'TRADING ANALYSIS - {day_name}', fontsize=24, fontweight='bold', y=0.95)
        ...
        if orders:
            ...
            times = [order['filled_at'].strftime('%H:%M') for order in sorted_orders[:10]]  # First 10
            symbols = [order['symbol'] for order in sorted_orders[:10]]
            
            plt.plot(range(len(times)), range(len(times)), 'o-', linewidth=3, markersize=8)
            ...
            for i, (time, symbol) in enumerate(zip(times, symbols)):
                plt.text(i, i, f'{symbol}\\n{time}', ha='center', va='bottom', 
...
WORST PERFORMER: {worst_stock[0]} (${worst_stock[1]:+.2f})
        ...
        color = 'lightgreen' if total_pnl > 0 else 'lightcoral' if total_pnl < 0 else 'lightyellow'
        ...
        if stock_pnl:
            table_data = []
            for symbol, pnl in sorted(stock_pnl.items(), key=lambda x: x[1], reverse=True):
                ...
                table_data.append([symbol, f'${pnl:+.2f}', trade_count, f'${avg_pnl:+.2f}', status])
            ...
            for i in range(1, len(table_data) + 1):
                ...
                color = 'lightgreen' if pnl > 0 else 'lightcoral' if pnl < 0 else 'lightyellow'
    ...
    def create_multi_page_report(self, output_dir='reports/daily'):
        ...
        try:
            ...
            pdf_path = os.path.join(output_dir, f'multi_page_trading_report_{timestamp}.pdf')
            
            with PdfPages(pdf_path) as pdf:
                print(f"Creating multi-page report with {len(all_data)} trading days...")
