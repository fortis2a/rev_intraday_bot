...
class SignalMonitor:
        ...
        for symbol in watchlist:
            ...
            try:
                ...
                if df.empty or len(df) < 26:
                    print(
                        f"  ❌ Insufficient data: {len(df) if not df.empty else 0} bars"
                    )
                    continue
                ...
                for strategy_name, strategy_class in strategy_classes.items():
                    try:
                        ...
                        if signal:
                            ...
                            print(
                                f"  🚨 {strategy_name.upper()}: {signal['action']} - {signal['reason']}"
                            )
                            ...
                            print(
                                f"     📊 Confidence: {confidence:.1f}% {'✅ PASS' if should_execute else '❌ FAIL'}"
                            )
                            ...
                            if existing_position:
                                print(
                                    f"     📍 Position: {existing_position['side']} {existing_position['qty']} shares"
                                )
                            else:
                            ...
                            execution_result = self.simulate_execution(
                                signal, trade_decision, existing_position
                            )
    ...
    def monitor_continuously(self, cycles=5):
        ...
        print(
            f"Short Selling: {'✅ ENABLED' if config.get('ENABLE_SHORT_SELLING', False) else '❌ DISABLED'}"
        )
        ...
        for cycle in range(1, cycles + 1):
            ...
            if signals:
                ...
                for signal in signals:
                    print(
                        f"   📌 {signal['symbol']}: {signal['action']} - Confidence pending execution"
                    )
            else:
