    ...
    try:
        ...
        if len(orders) > 0:
            ...
            for i, order in enumerate(orders[:5]):
                print(f"   {i+1}. {order.symbol} | {order.side} | {order.filled_qty} shares | ${order.filled_avg_price} | {order.filled_at}")
            ...
            print(f"\n📈 BY SYMBOL:")
            for symbol, data in sorted(by_symbol.items(), key=lambda x: x[1]['count'], reverse=True):
                print(f"   {symbol}: {data['count']} orders, {data['volume']:.0f} shares")
