    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            if df.empty or len(df) < 14:  # Reduced minimum for real-time trading
                print(f"ERROR: Insufficient data for {symbol}: {len(df) if not df.empty else 0} bars")
                continue
            ...
            if current_price < config['MIN_PRICE'] or current_price > config['MAX_PRICE']:
                print(f"BLOCKED: Price ${current_price:.2f} outside limits ${config['MIN_PRICE']}-${config['MAX_PRICE']}")
                continue
            ...
            for strategy_name, strategy_class in strategy_classes.items():
                try:
                    ...
                    strategy = strategy_class(symbol)  # All strategies need symbol parameter
            ...
            print(f"\nSUMMARY for {symbol}:")
            print(f"  - Confidence: {decision['confidence']:.1f}% ({'PASS' if decision['execute'] else 'FAIL'})")
