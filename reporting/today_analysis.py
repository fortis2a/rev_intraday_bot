...
class TodayAnalysis:
    ...
    def get_account_info(self):
        ...
        try:
            ...
            print(
                f"  💴 Day Trade Buying Power: ${float(account.daytrading_buying_power):,.2f}"
            )
    ...
    def analyze_trades(self, orders):
        ...
        print(
            f"{'Symbol':<8} {'Trades':<8} {'P&L $':<12} {'P&L %':<10} {'Volume $':<12}"
        )
        print("-" * 55)
        ...
        for symbol, trades in trades_by_symbol.items():
            ...
            if total_bought > 0 and total_sold > 0:
                ...
                print(
                    f"{status} {symbol:<7} {len(buys)+len(sells):<8} ${pnl:<11.2f} {pnl_pct:+.1f}%     ${total_volume:,.0f}"
                )
        ...
        if orders:
            ...
            print(f"\n⏰ TIME ANALYSIS:")
            print(
                f"  🌅 First Trade: {first_trade.filled_at.strftime('%H:%M:%S')} ({first_trade.symbol})"
            )
            print(
                f"  🌆 Last Trade: {last_trade.filled_at.strftime('%H:%M:%S')} ({last_trade.symbol})"
            )
