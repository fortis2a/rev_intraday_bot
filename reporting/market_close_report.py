...
class MarketCloseReportGenerator:
    ...
    def get_extended_orders(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Fallback: Found {len(today_orders)} orders filled today")
                self.logger.info(f"Fallback: Found {len(yesterday_orders)} orders filled yesterday")
    ...
    def _get_yesterday_summary(self, yesterday_activities):
        ...
        if not yesterday_activities:
            return {'total_trades': 0, 'total_volume': 0, 'net_pnl': 0, 'unique_symbols': 0}
    ...
    def _analyze_side_performance(self, df):
        ...
        side_stats['short_selling_stats'] = {
            ...
            'short_sell_ratio': total_short_sells / (total_sells + total_short_sells) if (total_sells + total_short_sells) > 0 else 0
        }
    ...
    def _perform_statistical_analysis(self, trades_df):
        ...
        holding_periods = trades_df.get('holding_period_minutes', pd.Series(dtype=float)).values
        ...
        basic_stats = {
            ...
            'win_rate': len(trades_df[trades_df['cash_flow'] > 0]) / len(trades_df) * 100,
            'avg_win': trades_df[trades_df['cash_flow'] > 0]['cash_flow'].mean() if len(trades_df[trades_df['cash_flow'] > 0]) > 0 else 0,
            'avg_loss': trades_df[trades_df['cash_flow'] < 0]['cash_flow'].mean() if len(trades_df[trades_df['cash_flow'] < 0]) > 0 else 0,
    ...
    def _analyze_overtrading(self, trades_df):
        ...
        if len(trades_df) > 1:
            for i in range(1, len(trades_df)):
                time_diff = (trades_df.iloc[i]['filled_at'] - trades_df.iloc[i-1]['filled_at']).total_seconds() / 60
                if time_diff < 5:
                    rapid_trades += 1
    ...
    def _detect_revenge_trading(self, trades_df):
        ...
        for i in range(1, len(trades_df)):
            ...
            if prev_trade['cash_flow'] < 0 and current_trade['value'] > prev_trade['value'] * 1.5:
    ...
    def _get_trade_summary(self, df):
        ...
        return {
            ...
            'short_sell_orders': len(df[df['side'] == 'sell_short']) if 'sell_short' in df['side'].values else 0,
    ...
    def _create_statistical_charts(self, trades_df, analysis_data):
        ...
        if 'holding_period_minutes' in trades_df.columns and not trades_df['holding_period_minutes'].isna().all():
            ...
            ax7.scatter(holding_periods, trades_df['cash_flow'], alpha=0.6, c=trades_df['cash_flow'], cmap='RdYlGn')
            ...
            ax7 = fig.add_subplot(gs[3, :2])
            ax7.text(0.5, 0.5, 'Holding Period Analysis\nNot Available for Activities Data', 
