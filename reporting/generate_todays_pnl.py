    ...
    try:
        ...
        if positions:
            for pos in positions:
                ...
                print(f"    Unrealized P&L: {pnl_sign}${unrealized_pnl:.2f} ({unrealized_pct:+.2f}%)")
                print()
        ...
        if today_orders:
            ...
            for order in today_orders:
                status_color = "✅" if order.status.name == "FILLED" else "⏳" if order.status.name == "ACCEPTED" else "❌"
                print(f"  {status_color} {order.symbol}: {order.side.name} {order.qty} @ {order.order_type.name} - {order.status.name}")
        else:
        ...
        try:
            ...
            if portfolio_history.equity:
                ...
                day_change_pct = (day_change / start_equity) * 100 if start_equity > 0 else 0
