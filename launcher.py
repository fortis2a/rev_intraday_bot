                ...
                if next_open_str and next_open_str != "None":
                    try:
                        ...
                        if time_diff.total_seconds() > 0:
                            ...
                            print(
                                f"[COUNTDOWN] Next Market Open: {next_open_et.strftime('%Y-%m-%d %H:%M:%S %Z')}"
                            )
                            print(
                                f"[COUNTDOWN] Time Until Open: {hours:02d}:{minutes:02d}:{seconds:02d}"
                            )
                        else:
            ...
            while True:
                try:
                    ...
                    if market_status == "CLOSED":
                        ...
                        print(
                            f"[LIVE TIME] {current_time.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                            ...
                            if next_open_str and next_open_str != "None":
                                try:
                                    ...
                                    if time_diff.total_seconds() > 0:
                                        ...
                                        print(
                                            f"[NEXT OPEN] {next_open_et.strftime('%Y-%m-%d %H:%M:%S %Z')}"
                                        )
                                        print(
...
"""
    ...
    def run_eod_analysis(self):
        ...
        try:
            ...
            if choice in ["y", "yes"]:
                ...
                result = subprocess.run(
                    [self.python_cmd, "scripts/eod_analysis.py"],
                    capture_output=True,
                    text=True,
                    timeout=300,
                )
                ...
                if result.returncode == 0:
                    ...
                    if dashboard_path.exists():
                        open_dashboard = (
                            input("\nOpen interactive dashboard? (Y/n): ")
                            .strip()
                            .lower()
                        )
                        if open_dashboard != "n":
                            try:
                                ...
                                print(
                                    f"[SUCCESS] 🌐 Dashboard opened: {dashboard_path}"
                                )
                            except Exception as e:
    ...
    def run_github_backup(self):
        ...
        try:
            ...
            if choice == "1":
                ...
                result = subprocess.run(
                    [self.python_cmd, "scripts/backup_system.py", "backup"],
                    capture_output=True,
                    text=True,
                    timeout=120,
                )
                ...
                if result.returncode == 0:
                    ...
                    print(
                        "[INFO] 📦 Repository: https://github.com/fortis2a/rev_intraday_bot"
                    )
                ...
                try:
                    result = subprocess.run(
                        [self.python_cmd, "scripts/backup_system.py", "schedule"],
                        timeout=None,
                    )  # No timeout for scheduler
                except KeyboardInterrupt:
    ...
    def start_live_trading_with_signals(self):
        ...
        try:
            ...
            print(
                f"[TIME] Current Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            ...
            if self.start_main_engine("LIVE"):
                ...
                if not enable_monitoring:
                    ...
                    print(
                        "[INFO] Engine is running independently. Check logs for status."
                    )
                ...
                if process:
                    try:
                        ...
                        print(
                            "[INFO] Engine running in background. Logs available in 'logs/' directory."
                        )
                        ...
                        while process.poll() is None:
                            ...
                            if cycle_count % 6 == 0:
                                ...
                                print(
                                    f"[STATUS] {current_time} - Engine running (PID: {process.pid}) - Cycle {cycle_count}"
                                )
                        ...
                        if choice == "1":
                            ...
                            if confirm in ["yes", "y"]:
                                ...
                                if self.start_main_engine("LIVE"):
                                    ...
                                    print(
                                        "[INFO] Engine restarted. Returning to main menu to avoid loops."
                                    )
                                    return
                                else:
                                ...
                                print(
                                    "[CANCEL] Restart cancelled - returning to main menu"
                                )
                                return
                        elif choice == "3":
            ...
            if log_files:
                ...
                with open(latest_log, "r", encoding="ascii", errors="replace") as f:
                    ...
                    for line in recent_lines:
                        ...
                        if (
                            line_content
                            and line_content not in self._last_shown_lines
                            and not line_content.endswith(
                                "Starting Intraday Trading Engine in LIVE mode"
                            )
                            and not line_content.endswith(
                                "Intraday trading engine stopped"
                            )
                        ):
        ...
        while True:
            try:
                ...
                if choice == "1":
                    ...
                    try:
                        result = subprocess.run(
                            [self.python_cmd, "test_enhanced_indicators.py"],
                            capture_output=False,
                            text=True,
                        )
                        if result.returncode == 0:
                            print(
                                "[SUCCESS] Enhanced indicators test completed successfully!"
                            )
                        else:
                            print(
                                "[WARNING] Enhanced indicators test completed with warnings"
                            )
                    except Exception as e:
