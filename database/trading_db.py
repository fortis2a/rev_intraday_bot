...
class TradingDatabase:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_activities_date ON trading_activities(trade_date)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_activities_symbol ON trading_activities(symbol)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_activities_time ON trading_activities(transaction_time)")
    ...
    def fetch_and_store_activities(self, target_date: date) -> Tuple[int, bool]:
        ...
        try:
            ...
            self.logger.info(f"Successfully stored {len(activities)} activities for {target_date}")
            return len(activities), True
            
        except Exception as e:
            error_msg = str(e)
            self.logger.error(f"Error fetching activities for {target_date}: {error_msg}")
    ...
    def _date_exists(self, target_date: date) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM trading_activities WHERE trade_date = ?", (target_date,))
    ...
    def _generate_daily_summary(self, target_date: date):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            total_trades, unique_symbols, total_volume, first_trade, last_trade, symbols = row
            ...
            try:
                ...
                if alpaca_result.get('error'):
                    self.logger.warning(f"Alpaca P&L fetch failed: {alpaca_result['error']}")
            ...
            """, (
                target_date, total_trades, unique_symbols, total_volume, cash_flow_pnl, 
                trading_pnl, alpaca_pnl, 0, 0, 0,  # positions data placeholder
                symbols, first_trade, last_trade, trading_hours
            ))
            
            conn.commit()
            self.logger.info(f"Generated daily summary for {target_date}: {total_trades} trades, "
                           f"Cash Flow: ${cash_flow_pnl:.2f}, Alpaca P&L: ${alpaca_pnl:.2f}")
    
    def _log_collection(self, target_date: date, count: int, status: str, error: Optional[str]):
    ...
    def get_daily_summary(self, target_date: date) -> Optional[Dict]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM daily_summaries WHERE trade_date = ?", (target_date,))
            row = cursor.fetchone()
    ...
    def get_date_range_summaries(self, start_date: date, end_date: date) -> pd.DataFrame:
    ...
    def backfill_historical_data(self, start_date: date, end_date: date = None):
        ...
        self.logger.info(f"Backfill complete: {success_count} successful, {error_count} errors")
